...
- [x] সমন্বয়ে `issue()` → `move()` মিউট্যান্ট চালিয়ে প্রমাণ করা ঘাটতির দাবিটা কামড়ায় — **কামড়ায়** (২৫ সেপ্টেম্বর)

  ⭐ ঘাটতিটা `move(floor: $difference)` দিয়ে লিখলে **মোট ঠিকই থাকে**, কেবল
  লটের হিসাব অক্ষত থেকে যায়। ⓘ ঠিক যে দাবিটার ধরার কথা, সেটাই লাল হলো:
  `test_a_shortage_comes_out_of_the_lot` — *"মাল মেঝে থেকে কমল অথচ লটের
  হিসাব অক্ষত — পরের বিক্রয় খালি লট থেকে বরাদ্দ করবে।"* ⚠️ বাকি ১৫টা দাবি
  সবুজই থেকে গেল, অর্থাৎ পাহারাটা **এই** জিনিসটাই মাপে, আশপাশের কিছু নয়।

- [x] `NoDoorTakesALineWorthNothingTest` চালানো (দশ দরজায় শূন্য-দর) — সবুজ (২৫ সেপ্টেম্বর)

  ⚠️ ফাইলটা `tests/Feature/Modules/Inventory/`-তে নেই, আছে
  `tests/Feature/Core/`-এ। ⛔ প্রথম রানটা *"Test file not found"* বলে
  **exit 0** দিয়ে থেমেছে — অর্থাৎ কিছু না চালিয়েও সবুজের মতো দেখাত।
  ⓘ চেকলিস্টে পথ না লেখা থাকলে পরের বার আবার একই ফাঁদ, তাই পথটা এখানেই।

- [x] `tests/Feature/Architecture` পুরোটা — **তিনটা লাল, তিনটাই অন্যের** (২৫ সেপ্টেম্বর)

  ⓘ `EveryRawQueryNamesItsCompany` → `InterCompanyService`: HEAD-এ কমিট করা।
  ⚠️ আন্তঃকোম্পানিতে **অন্য কোম্পানির ভাউচার পড়াটাই কাজ** (`in_voucher_id`),
  তাই `company_id` ফিল্টার বসালে ফাংশনটা সবসময় খালি ফিরত — ফিল্টার নয়,
  কারণসহ ছাড় দরকার। ⓘ হিসাবের শব্দার্থের সিদ্ধান্ত, তাই abos-13-কে
  বিশ্লেষণসহ পাঠানো হয়েছে, নিজে বসাইনি।

  ⓘ `EveryRouteIsGuarded` → `inventory.qc.dispose` অনুমতি চায় না; কন্ট্রোলার
  ও রুট দুইটাই ` M` — সহকর্মীর চলতি কাজ, ছোঁয়া হয়নি।

  ⓘ `TheBuiltAssetsMatchTheSource` → `public/build` বাসি, কারণ
  `counter/direct-sale.js` বদলেছে। ⚠️ abos-13 ডিপ্লয়ের আগে একবারে বাঁধবেন,
  তাই কমিট করা হয়নি — এটাই তাঁর নির্দেশ।

- [x] কমিট, আর হ্যাশ abos-41-কে — `000802eb` (২৫ সেপ্টেম্বর)
