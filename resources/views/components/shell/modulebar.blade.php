...
<div data-module-bar
    ...
    <nav @class(['flex min-w-0 flex-1 items-center', 'overflow-x-auto' => ! $grouped])
        ...
        @if (! $grouped)
            ...
            @foreach ($tabs as $tab)
                {{-- ⚠️ নিচের `-ms-px`, `first:rounded-s-*` ও `last:rounded-e-*`
                     **আর খাটে না** — ১৫ সেপ্টেম্বর ২০২৬-এ মালিক ঘরগুলো
                     ত্রিমাত্রিক করতে বলেছেন, আর `app.css`-এর
                     `.modulebar-cell` (যেটা `@layer`-এর বাইরে বলে
                     ইউটিলিটিকে হারায়) এখন নিজেই ফাঁক ও গোল কোণা বসায়।

                     ⓘ ক্লাসগুলো রেখে দেওয়া হয়েছে ইচ্ছাকৃতভাবে: CSS-টা
                     কোনোদিন সরলে ঘরগুলো আগের জোড়া-লাগা চেহারাতেই ফিরবে,
                     ভেঙে পড়বে না। ⛔ কিন্তু কেউ যেন পড়ে ভেবে না বসেন
                     ওগুলোই আজকের চেহারা ঠিক করছে — করছে না। --}}
                <a href="{{ $tab['url'] }}"
                   @class([
