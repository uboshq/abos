...
<body class="flex min-h-dvh flex-col">
    ...
    <div class="flex min-h-dvh flex-1">
        {{-- বাঁ পাশের তালিকা — `rail` আর `quiet`, দুইটাতেই।

             দুইটার তফাত তালিকায় নয়, **উপরে**: `rail` তালিকার সাথে
             একটা টপবারও রাখে, `quiet` রাখে না। তাই শর্তটা "রেল কি না"
             নয়, "উপরে বার আছে কি না" — আর সেটা নিচে আলাদা করে বলা। --}}
        @if (in_array($navPlacement, ['rail', 'quiet'], true))
            <x-shell.sidebar :menu="$menu ?? []" />
        @endif

        <div class="flex min-w-0 flex-1 flex-col">
            {{-- `quiet` ছাড়া সব রূপে উপরের বার।

                 ── কেন একটা রূপে বারটা একেবারেই থাকে না ──────────────
                 Linear-এর পরিচয়টাই ওই বারের **অনুপস্থিতি** — পাতা
                 পর্দার একদম উপর থেকে শুরু হয়। বারটা রেখে কেবল রং
                 বদলালে যা দাঁড়ায় সেটা Linear নয়, Linear-এর রঙে আঁকা
                 আর সবাই।

                 বারের নিয়ন্ত্রণগুলো মুছে যায়নি — `chrome/linear.blade.php`
                 ওগুলো সাইডবারের পায়ে বসায়।

                 ── কিন্তু সরু পর্দায় বারটা থাকতেই হয় ─────────────────
                 সাইডবার `lg:flex` (১০২৪px+), bottom-nav `md:hidden`
                 (৭৬৮px-এর নিচে)। বারটা একেবারে তুলে নিলে **৭৬৮–১০২৩px-এ
                 কোনো নেভিগেশনই থাকত না**।

                 তাই `quiet`-এ বারটা আসে `narrow` হয়ে — `lg:hidden`,
                 অর্থাৎ ঠিক ততটুকু চওড়ায় যতক্ষণ সাইডবার নেই। Linear
                 নিজেও সরু পর্দায় হেডার দেখায়, তাই নকলটা ভাঙে না। --}}
            @if ($navPlacement === 'quiet')
                <x-shell.topbar :menu="$menu ?? []" narrow />
            @else
            ...
            {{--
                টপবারের নিচের বার — টপবার পুরো অ্যাপের, এটা এই মডিউলের।

                ── কেন দুইটা সম্ভাবনা, ১৩ সেপ্টেম্বর ২০২৬ ──────────────────
                ⭐ মালিকের চাওয়ায় বারটা এখন **চলতি মডিউলের পর্দাগুলোর
                ট্যাব সারি** (`modulebar`) — আগের breadcrumb নয়। সেটা
                বলত "আমি কোথায়"; এটা তার সাথে বলে "পাশে আর কী আছে"।

                ⛔ **কেবল ABOS রূপে**, বাকি সাতটায় নয় — মালিকের স্পষ্ট
                নির্দেশ (১৩ সেপ্টেম্বর ২০২৬): *"তুমি শুধু ABOS theme-এ কাজ
                করবে, অন্যগুলোতে না।"*

                ⓘ কোডে ABOS-এর নাম `navy` (`core.ui.navy` অনুবাদে "ABOS",
                আর ব্লার্ব *"An ABOS original — the brand teal"*)। ⚠️ নামটা
                বদলানো হয়নি ইচ্ছাকৃতভাবে — `Ui::DEFAULT` ও ব্যবহারকারীদের
                সংরক্ষিত `ui` ঘরে ওটাই বসা, আর নাম বদলালে সবার রূপ নীরবে
                ডিফল্টে নেমে যেত।

                ⓘ বাকি রূপগুলো আগের breadcrumb-ই পায়, অর্থাৎ এই বদলে
                তাদের কিছুই বদলায়নি। ⭐ আর তার একটা পার্শ্ব-লাভও আছে:
                `salesforce` ও `suite` নিজেরাই একটা ট্যাব সারি আঁকে
                (`chrome/*`-এর `page-head`), তাই ওখানে এটা বসালে দুইটা
                ট্যাব বার হত — একটার নিচে আরেকটা।
            --}}
            @if ($shellLook === 'navy')
                <x-shell.modulebar :menu="$menu ?? []" />
            @else
            ...
            {{-- ⚠️ উপরের ফাঁকটা অর্ধেক — মালিকের নির্দেশ (৩ সেপ্টেম্বর ২০২৬):
                 *"mark kora faka jayga zoto tuku paro komaw"*।

                 ── কেন এখানে, পাতায় নয় ──────────────────────────────────
                 ব্রেডক্রাম্বের পটি আর প্রথম কার্ডের মাঝের ফাঁকা জায়গাটা
                 **কোনো পাতার নিজের নয়** — এটা লেআউটের `pt`। এক পাতায়
                 ঠিক করলে বাকিগুলো আলাদা দেখাত, আর কেউ পরে ভাবতেন কোনটা
                 ঠিক।

                 ⚠️ শূন্য করা হয়নি: পটিটা আঠালো, আর কার্ড একদম গা ঘেঁষে
                 বসলে স্ক্রল করার সময় **কার্ডের উপরের কিনারা পটির নিচে
                 সেঁটে যেত** — দেখে মনে হত কাটা পড়েছে। এক ধাপই যথেষ্ট।

                 ⓘ পাশে-নিচে অপরিবর্তিত; কেবল উপরের ফাঁকটাই মার্ক করা
                 হয়েছিল, আর যেটা চাওয়া হয়নি সেটা বদলানো হয়নি। --}}
            <main id="main"
                  class="min-w-0 flex-1 px-3 pt-2 pb-[calc(var(--spacing-bottom-nav)+1.5rem)]
                ...
                <div class="mx-auto w-full max-w-(--spacing-content-max)">
                    ...
                    {{-- ফিওরির নোঙর-পটি — লম্বা রেকর্ডের অংশগুলোয় লাফানো।

                         শিরোনামের নিচে, লেখার উপরে: ঠিক যেখানে Fiori রাখে।

                         লেআউটে বসানো, প্রতিটা পাতায় নয় — কম্পোনেন্টটা পাতার
                         `<section>`-গুলো নিজেই পড়ে, তাই কোনো পাতাকে কিছু
                         জানাতে হয় না। দুইটার কম অংশ হলে সে নিজেই আঁকা বন্ধ
                         রাখে, ফলে তালিকার পর্দাগুলোয় কিছুই বসে না। --}}
                    <x-ui.anchor-nav />
            ...
            {{-- ── রূপের নিজের নিচের পট্টি ──────────────────────────────

                 ⓘ `<main>`-এর **বাইরে**, কিন্তু কাজের কলামের **ভিতরে** —
                 আর জায়গাটা ইচ্ছাকৃত। বাইরে, কারণ পট্টিটা কনটেন্টের সাথে
                 স্ক্রল করে চোখের আড়ালে যাওয়ার কথা নয়; কলামের ভিতরে,
                 কারণ ওটা **এই তালিকার** যোগফল, গোটা অ্যাপের নয় (সেটা
                 স্ট্যাটাস বার, আরও নিচে)।

                 ⚠️ পর্দা এখানে কিছু বসায় না। যে তালিকার যোগফল আছে সে
                 `@push` দিয়ে ঘোষণা করে, আর **দেখানো হবে কি না সেটা রূপ
                 ঠিক করে** — তাই "পর্দা তার রূপ জানে না" নিয়মটা টেকে।

                 ⓘ যে রূপের নিজের ফাইল নেই, তার জন্য এখানে কিছুই আঁকা
                 হয় না — `chrome` কেবল ফাইলটা থাকলে ডাকে। --}}
            <x-shell.chrome region="page-foot" :menu="$menu ?? []" />
        </div>
    </div>
    ...
    {{-- খোঁজার প্যালেট — টপবার ও নিচের নেভ দুইটাই `open-command-center`
         পাঠায়, আর এতদিন কেউ শুনত না। ⓘ সব রূপেই বসে, কারণ বোতামটাও
         সব রূপে আছে; এটা ABOS-এর আলাদা সাজ নয়, একটা অনুপস্থিত তার। --}}
    <x-shell.command-center />
    ...
    {{-- পাতার নিজের ফর্ম, যেগুলো **কোনো ফর্মের ভেতরে বসতে পারে না**।

         ── কেন এই স্ট্যাকটা লাগল ────────────────────────────────────────
         তালিকার পর্দাগুলো তাদের টুলবার একটা `<form method="GET">`-এর
         ভেতরে রাখে — ছাঁকনি ও খোঁজা তাতেই কাজ করে, আলাদা JavaScript
         ছাড়াই।

         কিন্তু HTML-এ ফর্মের ভেতরে ফর্ম বসে না। সংরক্ষিত দৃশ্যের মেনুতে
         POST ফর্ম বসানোর পর ব্রাউজার **গোটা মেনুটাকেই ফর্মের বাইরে সরিয়ে
         দিয়েছিল** — আর তাতে সে তার Alpine `x-data`-র বাইরে চলে গিয়ে
         `open` পেত না। ফল: মেনুটা DOM-এ আছে, মাপ ০×০, ক্লিক করলে কিছুই
         হয় না। আরেকটা মৃত বোতাম, এবার আমার নিজের বানানো।

         ২৭ অগাস্ট ২০২৬-এ ধরা পড়েছে ব্রাউজারে, PHPUnit-এ নয় — সাতটা
         টেস্ট সবুজ ছিল, কারণ ওরা HTTP-তে POST করে, DOM পার্স করে না।

         ── সমাধানটা মান-সম্মত, JavaScript নয় ────────────────────────────
         ফর্মগুলো এখানে বসে, গোটা পাতার শেষে, কোনো ফর্মের ভেতরে নয়। আর
         মেনুর বোতামগুলো `form="<id>"` দিয়ে তাদের ডাকে — HTML-এর নিজের
         নিয়ম, যেটা ঠিক এই কাজের জন্যই আছে। --}}
