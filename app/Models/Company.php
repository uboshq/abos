...
class Company extends Model
{
    ...
    /**
     * কোডটা এখনো নিজে থেকে বদলানো যায় কি না — কেউ কিছু না জানলেও নিরাপদ।
     *
     * ⓘ একটা নম্বরও ইস্যু হয়নি আর খাতায় একটা সারিও নেই — তখন কোড বদলানোয়
     * কোনো ঝুঁকি নেই, কারণ ঐ কোড এখনো কোনো কাগজে যায়নি।
     */
    ...
    /**
     * সুপার অ্যাডমিন কি কোডটা বদলাতে পারেন — কাগজ বেরিয়ে যাওয়ার পরেও?
     *
     * ── কেন দরজাটা রাখা হলো, ২০ সেপ্টেম্বর ২০২৬ ────────────────────
     * প্রথম নিয়মটা ছিল: একটা সারি খাতায় ঢুকলেই কোড চিরতরে তালাবদ্ধ।
     * ⚠️ কিন্তু মালিকের আসল কোম্পানিতে পড়ে ছিল কেবল **পরীক্ষার দুইটা
     * সারি** (একটা গ্রাহকের খোলা ব্যালেন্স), আর তাতেই আসল ব্যবসার নামের
     * সাথে ভুল কোড চিরকাল বসে থাকত।
     *
     * ⓘ আর কোডটা আসলে নম্বরের ভিতরে বসেও না — উপসর্গ আসে ডকুমেন্টের ধরন
     * থেকে (`INV`, `CUS`, `TIL`)। ঝুঁকিটা ছাপা কাগজের মাথায় আর রপ্তানি
     * ফাইলে, অর্থাৎ **বাইরে যাওয়া কাগজে**।
     *
     * ⛔ তাই দরজাটা বন্ধ নয়, কিন্তু সরুও: কেবল সুপার অ্যাডমিন, আর নিশ্চিত
     * করতে পুরনো কোডটা হুবহু লিখতে হয় — ঠিক বছর খোলার মতো। পুরনো কোডটা
     * অডিটে থেকে যায়, তাই ছাপা কাগজের সাথে মেলাতে গেলে ইতিহাসটা পাওয়া যায়।
     */
    public function codeChangeNeedsSuperAdmin(): bool
    {
        return ! $this->canChangeCode();
    }
