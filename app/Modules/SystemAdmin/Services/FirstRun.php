 ...
 *   CompanyProvisioner::grantAccess()  কোম্পানির রোল বসায়, তারপর দেয়
 *
 * ── ⚠️ এই ঘরটা ৭ সেপ্টেম্বর ২০২৬-এ বদলেছে ────────────────────────────
 * আগে এখানে লেখা ছিল রোলটা **এখানেই** বানাতে হয়, আর কারণও লেখা ছিল:
 * `keepOwnerComplete()` রোল না পেলে চুপচাপ `0` ফেরত দিত ("সিডার বসাবে"
 * — কিন্তু প্রোডাকশনে সিডার চলে না)।
 *
 * ⓘ teams চালু হওয়ার পর দুইটাই বদলেছে: রোল এখন **কোম্পানির ভেতরের
 * জিনিস** (তাই কোম্পানির আগে বানানো অসম্ভব), আর `keepOwnerComplete()`
 * এখন না পেলে **নিজেই বসায়**।
 *
 * ⛔ পুরনো ক্রমটা রেখে দিলে দুইটার একটা হত: `company_id = null` (রোলটা
 * কারও নয়), নয়তো *"A role `owner` already exists"* বলে থেমে যাওয়া।
 * ⚠️ সুইটে দুইটাই ঘটেছে।
 *
...
final class FirstRun
{
     ...
     * দরজাটা কি আর খোলা?
     *
     * শর্তটা **"owner একজনই"** নয়, শর্তটা **"এই দরজা দিয়ে একবারই ঢোকা
     * যায়"** — আর সেটা মাপা হয় সবচেয়ে সরল প্রশ্নে: এই ইনস্টলে কি
     * একটাও ব্যবহারকারী আছেন?
     *
     * ── ⚠️ এখানে আগে উল্টো কথা লেখা ছিল, ১৩ সেপ্টেম্বর ২০২৬ ───────────
     * এই অনুচ্ছেদে লেখা ছিল *"একটা প্রতিষ্ঠানে পরে **যত খুশি owner**
     * থাকতে পারেন"*। ⛔ মালিকের সিদ্ধান্তে সেটা আর সত্য নয়: একটা
     * কোম্পানিতে সক্রিয় owner **ঠিক একজন**, আর রোলটা সরানো যায় না,
     * কেবল হস্তান্তর হয় ([[App\Core\Services\Ownership]])।
     *
     * ⓘ বাক্যটা কেবল মুছে দেওয়া হয়নি, কারণ তাহলে পরের জন ভাবতেন
     * নিয়মটা কোনোদিন ছিলই না। ⚠️ আর রেখে দিলে আরও খারাপ হত — এই রিপোতে
     * বারবার দেখা গেছে যে **মানুষ মন্তব্যটাই বিশ্বাস করেন, কোডটা নয়**।
     *
     * ⓘ তবে এই পদ্ধতিটার নিজের উত্তর বদলায়নি: দরজাটা তখনই খোলা যখন
     * একজন ব্যবহারকারীও নেই, তাই প্রথম owner নির্বিঘ্নেই বসেন —
     * তালাটা "ইনি ছাড়া আর কেউ owner নন" শর্তে চলে, আর তখন কেউ নেই।
     *
    ...
    public function open(array $data): User
    {
        return DB::transaction(function () use ($data): User {
            ...
            /*
             * ── ⛔ ক্রমটা teams-এর দিনে উল্টে গেল, ৭ সেপ্টেম্বর ২০২৬ ────────
             *
             * এখানে আগে লেখা ছিল: *"অনুমতি ও রোল আগে, ব্যবহারকারীর আগে।
             * ক্রমটা বদলানো যাবে না।"* ⓘ কথাটা সত্যি ছিল যতদিন রোল
             * বিশ্বজনীন — তখন কোম্পানির আগেই রোল বানানো **যেত**।
             *
             * ⚠️ teams-এর পর রোল **কোম্পানির ভেতরের জিনিস**, তাই কোম্পানি
             * তৈরির আগে সেটা বানানোই অসম্ভব। ⛔ চেষ্টা করলে দুইটার একটা
             * হত: `company_id = null` (কারও নয়), নয়তো পরে
             * *"A role `owner` already exists"* বলে থেমে যাওয়া — দুইটাই
             * ঘটেছে, সুইটে।
             *
             * ⭐ অনুমতিগুলো (কোম্পানি-নিরপেক্ষ) এখানেই বসে, আর **রোল বসে
             * `grantAccess()`-এ** — কোম্পানি তৈরি হওয়ার পর, তার নিজের
             * প্রসঙ্গে।
             */
