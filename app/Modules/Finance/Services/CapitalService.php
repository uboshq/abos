...
final class CapitalService
{
    ...
    public function positions(?string $profit = null): array
    {
        ...
        foreach ($out as $i => $position) {
             ...
             *
             * ⚠️ অংশ % না থাকলে `null`, শূন্য নয়: *"অংশ ঠিক হয়নি"*
             * আর *"অংশ নেই"* এক কথা নয়। ⓘ ১৯ সেপ্টেম্বর থেকে অংশ
    ...
    /**
     * যাঁর চুক্তির অংশ লেখা নেই, তাঁর অংশ তাঁর মূলধন থেকে — ১৯ সেপ্টেম্বর ২০২৬।
     *
     * ── ⛔ কী ঘটছিল ────────────────────────────────────────────────────
     * মালিক পর্দায় দেখলেন "অংশ %" আর "চলতি লাভের অংশ" দুটোই খালি, আর
     * বললেন: *"eta to auto hisab kore bosar kotha"*। ⓘ অংশ আসত কেবল জমার
     * সময় হাতে লেখা `share_percent` থেকে; না লিখলে ড্যাশ, আর তাই লাভের
     * অংশও ড্যাশ — একমাত্র মালিকের ব্যবসাতেও, যেখানে উত্তর স্পষ্ট ১০০%।
     *
     * ── ⭐ নিয়ম ─────────────────────────────────────────────────────────
     * ১. হাতে লেখা (চুক্তির) অংশ থাকলে সেটাই — অংশীদারি চুক্তি মূলধনের
     *    অনুপাত মানতে বাধ্য নয়, আর সেটা মানুষের সিদ্ধান্ত।
     * ২. বাকিরা বাকি শতাংশটা (১০০ − চুক্তির মোট) ভাগ করেন নিজেদের **বাকি
     *    মূলধনের** (দিয়েছেন − তুলেছেন) অনুপাতে।
     * ⚠️ বাকি মূলধন শূন্য বা ঋণাত্মক হলে ওই মানুষটার অনুপাত শূন্য — যিনি
     * সব তুলে নিয়েছেন তিনি লাভের ভাগ পান না, আর ঋণাত্মক অংশ ধরলে অন্যদের
     * ভাগ ১০০-র বেশি হয়ে যেত।
     *
     * ⓘ `share_source` বলে সংখ্যাটা কোথা থেকে এল — `agreed` (চুক্তি) না
     * `capital` (মূলধন থেকে হিসাব) — যাতে পর্দা দুইটাকে আলাদা দেখাতে পারে।
     *
     * @param  list<array<string, mixed>>  $out
     * @return list<array<string, mixed>>
     */
    private function fillSharesFromCapital(array $out): array
    {
        $agreed = '0';
        $pool = '0';
        ...
        foreach ($out as $position) {
            if ($position['share'] !== null) {
                $agreed = bcadd($agreed, $position['share'], 4);
            } elseif (bccomp($position['net'], '0', 4) > 0) {
                $pool = bcadd($pool, $position['net'], 4);
            }
        }

        $left = bcsub('100', $agreed, 4);

        if (bccomp($pool, '0', 4) <= 0 || bccomp($left, '0', 4) <= 0) {
            return $out;
        }
