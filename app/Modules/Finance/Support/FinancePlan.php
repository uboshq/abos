...
final class FinancePlan
{
     ...
     */
    public const RECONCILED_ON = '2026-09-23';
        ...
        return [
            [
                ...
                'items' => [
                    ...
                    /* ⭐ ২১ সেপ্টেম্বর ২০২৬ — পর্দাটা বানানো হয়ে গেছে। */
                    ['ঝুঁকির ড্যাশবোর্ড', 'finance.risk', null],
            ...
            [
                ...
                'items' => [
                    ...
                    /* ⭐ ২১ সেপ্টেম্বর ২০২৬ — আর টীকাটাও বদলাল।
                       ⓘ আগে লেখা ছিল "এখন ফেরতের কাগজ দিয়ে", কিন্তু ফেরত
                       মানে মাল নড়ে। ⛔ দাম ভুল বসা বা মাল নষ্ট হয়ে ছাড় —
                       এগুলোতে গুদামে কিছুই বদলায় না, আর ফেরতের কাগজ কাটলে
                       স্টক মিথ্যা বলত। */
                    ['ডেবিট ও ক্রেডিট নোট', 'accounts.note.index', null],
            ...
            [
                ...
                'items' => [
                    ...
                    /* ⭐ ২১ সেপ্টেম্বর ২০২৬।
                       ⓘ আলাদা রুট নেই — দরজাটা মিলকরণের পর্দার ভিতরে, আর সেটা
                       ইচ্ছাকৃত: সাধারণ ইমপোর্ট পর্দায় যেতে লাগে
                       `system_admin.import.manage`, অর্থাৎ নতুন কোম্পানি বসানোর
                       ক্ষমতা — অথচ স্টেটমেন্ট তোলা মাসের রোজকার কাজ।
                       ⚠️ এখান থেকে একটাও দাখিলা বইয়ে যায় না: ব্যাংক "SERVICE
                       CHARGE 230" লিখলে সেটা কোন খাতে যাবে ব্যাংক জানে না। */
                    ['ব্যাংক স্টেটমেন্ট আমদানি', 'accounts.reconciliation.index', 'মিলকরণের পাতা থেকে'],
            ...
            [
                ...
                'items' => [
                    ...
                    /* ⭐ ২১ সেপ্টেম্বর ২০২৬ — পর্দাটা বানানো হয়ে গেছে। */
                    ['বিনিয়োগের রিটার্ন', 'finance.investment.returns', null],
                    /*
                     * ⛔ লাইনটা "পরের ধাপ" বলত, অথচ পর্দাটা তৈরি — ২২ সেপ্টেম্বর ২০২৬।
                     *
                     * ⓘ [[ProfitDistributionController]], চারটা রুট, নিজের ভিউ ফোল্ডার
                     * আর দুইটা মাইগ্রেশন (`acc_profit_shares`) — সবই ছিল।
                     *
                     * ⚠️ [[EveryFinanceScreenIsOnTheMapTest]] এটা ধরেছিল আর **লাল হয়ে
                     * বসে ছিল** — অর্থাৎ পাহারাটা অন্ধ ছিল না, কেউ চালায়নি। ⛔ আর
                     * ততদিন মানচিত্র কম বলে গেছে: মালিক পড়তেন "অংশীদারি হলে করা হবে",
                     * অথচ কাজটা করাই আছে।
                     */
                    ['লাভ ভাগাভাগি', 'finance.profit.index', null],
                ],
            ],
            [
                'no' => '১৩',
            ...
            [
                ...
                'items' => [
                    ...
                    /*
                     * ⛔ লাইনটা "বাকি" বলত, অথচ খবরটা যায় — ২৩ সেপ্টেম্বর ২০২৬।
                     *
                     * ⓘ [[DueNotices]] আর `abos:money-due` তৈরি ও সূচিতে বসানো
                     * (`a2933e1c`), আর আজ রাতে বিজ্ঞপ্তির ইঞ্জিনটাও ঘণ্টার পাশে
                     * ইমেইল পাঠাতে শিখেছে। ⚠️ টীকাটা লেখা হয়েছিল তার আগে, আর
                     * তারপর কেউ ফিরে এসে কাটেনি।
                     *
                     * ⓘ লিংকটা বিজ্ঞপ্তির সেটিংসে যায়, কারণ খবরটার **নিজের কোনো
                     * পর্দা নেই** — ওটা সূচিতে চলে আর ঘণ্টায় ও ইমেইলে পৌঁছায়।
                     * ⛔ কোনো তালিকা-পাতার দিকে পাঠালে মানচিত্র এমন একটা পর্দার
                     * কথা বলত যা এই কাজটা করে না — আর সেটা "বাকি" লেখার চেয়েও
                     * খারাপ, কারণ ওটা নির্দিষ্ট করে ভুল বলে।
                     */
                    ['মেয়াদপূর্তির আগাম খবর', 'notifications.settings', 'সূচিতে `abos:money-due`, খবর যায় ঘণ্টায় ও ইমেইলে'],
                ],
            ],
            /*
            ...
            [
                ...
                'items' => [
                    ...
                    /* ⭐ ২১ সেপ্টেম্বর ২০২৬ — হয়ে গেছে।
                       ⓘ আলাদা রুট নেই: জোড়াটা হাতধারের নিজের পাতাতেই বসে,
                       কারণ প্রশ্নটা ঐ ধারের — "এই মানুষটা কি আমাদের গ্রাহক
                       বা সরবরাহকারীও?" ⚠️ আলাদা পর্দা হলে কেউ খুঁজত না। */
                    ['পক্ষের সাথে জোড়া', 'finance.hand_loan.index', 'হাতধারের পাতায়'],
                    /* ⓘ উপরের §১৪ক-এর একই কারণ — [[DueNotices]] হাতধারের তাগাদাও পাঠায়। */
                    ['মনে করিয়ে দেওয়া', 'notifications.settings', 'সূচিতে `abos:money-due`, খবর যায় ঘণ্টায় ও ইমেইলে'],
                ],
            ],
            /*
            ...
            [
                ...
                'items' => [
                    ...
                    /* ⭐ ২১ সেপ্টেম্বর ২০২৬ — হয়ে গেছে।
                       ⓘ সম্পদের নিজের পাতা থেকেই সরানো যায়, আর সঞ্চিত
                       অবচয়টাও সাথে যায় — নাহলে নতুন শাখায় জিনিসটা
                       নতুনের দামে বসত। */
                    ['সম্পদ স্থানান্তর', 'accounts.asset.index', 'সম্পদের পাতায়'],
                ],
            ],
            [
                'no' => '১৬',
            ...
            [
                ...
                'items' => [
                    /* ⭐ ২১ সেপ্টেম্বর ২০২৬।
                       ⚠️ খতিয়ানে আলাদা `project_id` কলাম বসানো হয়নি: প্রতিটা
                       সারি হ্যাশ-শিকলে বাঁধা, আর সই করা ঘরের তালিকায় নতুন নাম
                       ঢোকালে **আগের প্রতিটা সারির হ্যাশ অবৈধ** হয়ে যেত — খাতা
                       "বদলানো হয়েছে" বলে চিৎকার করত, অথচ কিছুই বদলায়নি।
                       ⓘ তাই বিদ্যমান খরচের কেন্দ্র মাত্রাটাই ব্যবহার করা; ডিপোতে
                       প্রকল্প আর খরচের কেন্দ্র একই জিনিস। প্রকল্পকে সত্যিই আলাদা
                       মাত্রা বানানো মালিকের সিদ্ধান্ত, আর তার সাথে শিকলের একটা
                       যুগ-বদল লাগবে। */
                    ['প্রকল্পভিত্তিক খতিয়ান', 'accounts.report.show:project-ledger', null],
                ],
            ],
            [
                'no' => '১৯',
