...
final class SalesInvoiceService
{
     ...
     * এই মন্তব্যটাই আগে লেখা ছিল: *"সীমাটা **বাকির** সীমা। টাকা হাতে
     * দিয়ে মাল নিলে কারও ধার বাড়ে না।"* ⚠️ **কিন্তু কোডে ওটা ছিল না** —
     * `wouldExceedCreditLimit()`-এ যেত বিলের **পুরো** অঙ্ক, আর কাউন্টারে
     * গোনা টাকাটা বসত তার **পরে**। ⓘ অর্থাৎ নিয়মটা মন্তব্যে ছিল, কোডে নয়।
     *
     * মন্তব্যের শেষ লাইনটা নিজেই স্বীকারোক্তি ছিল: *"নগদ বিক্রিতেও এক
     * মুহূর্তের জন্য পাওনা জন্মায়। সেটা সামলাতে `block_over_limit` বন্ধ
     * রাখার পথ খোলা আছে।"* ⛔ কিন্তু ওটা সামলানো নয় — **পুরো নিয়মটা বন্ধ
     * করে দেওয়া**। একটা পাহারা যাকে কাজ করতে দিলে নগদ বিক্রিও আটকায়,
     * সে পাহারা কেউ চালু রাখেন না।
     *
     * ── ⚠️ কেন এটা আজ ধরা পড়ল ──────────────────────────────────────
     * মালিকের নিয়ম: *"লিমিট নাই, বিল হবে না।"* ⓘ সেটা চালু হয়
     * `customer.zero_limit_blocks` সুইচে। কিন্তু সুইচটা টিপলে শূন্য মানে
     * শূন্য — আর তখন **পুরো টাকা হাতে দিয়েও** কেউ মাল নিতে পারতেন না,
     * কারণ যাচাইটা দেখত বিলের অঙ্ক, পরিশোধের নয়। ⛔ ডিপোর কাউন্টার আর
     * POS দুইটাই বন্ধ হয়ে যেত, আর সুইচটা কোনোদিন টেপাই যেত না।
     *
     * ── ⭐ এখন ──────────────────────────────────────────────────────
     * যাচাইটা দেখে **যতটুকু বাকি থাকছে** — বিল বাদ এই মুহূর্তে গোনা টাকা।
     * ⓘ নগদে পুরো দাম দিলে বাকির অঙ্ক শূন্য, তাই সীমার প্রশ্নই ওঠে না;
     * আধা দিলে কেবল **বাকি অর্ধেকটা** সীমার সাথে মেলে।
     *
     * @param  string  $payingNow  এই বিলের সাথেই যে টাকা গোনা হচ্ছে।
     *                             ⚠️ ডিফল্ট `'0'` ইচ্ছাকৃত: যে পথ টাকার
     *                             কথা বলে না, সেটা পুরোপুরি বাকি বিল —
     *                             আর সেখানে আগের আচরণই সঠিক।
     */
    ...
    public function create(array $data, array $lines): SalesInvoice
    {
        ...
        return DB::transaction(function () use ($data, $lines) {
             ...
             *
             * ── যা ঘটছিল (মাপা, ৩ সেপ্টেম্বর ২০২৬) ──────────────────────
             * কাউন্টারের ঘরটা `preview()` দিয়ে আগে থেকে ভরা থাকে।
             * ব্যবহারকারী কিছু না বদলে সেভ করলে এই লাইনটা ওটাকে হাতে-লেখা
             * ধরে নিত, **আর সিরিজ এক ধাপও এগোত না**। ফল দিনের দ্বিতীয়
             * বিক্রয়েই: একই নম্বর আবার, আর ঘর খালি করলে `next()`-ও সেই
             * নম্বরই দিত — **ডাটাবেসের ইউনিক ইনডেক্সে ৫০০**।
             *
     ...
     */
    /**
     * @param  string  $payingNow  এই বিলের সাথেই কাউন্টারে গোনা টাকা।
     *                             ⓘ কেবল ধারের সীমা মাপতে ব্যবহৃত — আদায়ের
     *                             কাগজ এটা বসায় না, সেটা ডাকা পক্ষের কাজ।
     *                             ⚠️ ঘরটা না দিলে বিলটা পুরোপুরি বাকি ধরা হয়।
     */
    public function confirm(SalesInvoice $invoice, string $payingNow = '0'): SalesInvoice
    {
         ...
         */
        $this->assertWithinCreditLimit($invoice, $payingNow);
    ...
    /**
     * সিরিজের পরের **খালি** নম্বর।
     *
     * ── কেন "খালি" শব্দটা লাগল (মাপা, ৩ সেপ্টেম্বর ২০২৬) ────────────
     * ⚠️ সিরিজ আর বাস্তব বিলের মাঝে ফারাক তৈরি হতে পারে — পুরনো ডেটা
     * আমদানি, হাতে বসানো নম্বর, বা এই রিপোতেই যে বাগটা ছিল (পর্দার ভরে
     * রাখা নম্বরে সিরিজ এগোত না)। ফারাক হলে `next()` এমন একটা নম্বর
     * ফিরিয়ে দিত যেটা **আগেই বসে আছে**, আর তখন ডাটাবেসের ইউনিক ইনডেক্স
     * **৫০০ পাতা** দেখাত — কাউন্টারে দাঁড়ানো লোকের সামনে।
     *
     * ⭐ তাই সিরিজ নিজে থেকে সামনে হাঁটে যতক্ষণ না খালি নম্বর পায়।
     * নম্বর অনন্য ও ক্রমবর্ধমান থাকে, আর যেখানে ফাঁক পড়ে সেখানে নম্বরটা
     * সত্যিই ব্যবহৃত ছিল।
     *
     * ⓘ সীমা ৫০ — অসীম লুপ নয়। এতেও না মিললে সমস্যাটা সিরিজের নয়,
     * আর তখন চুপ করে থাকার চেয়ে থেমে যাওয়াই ভালো: ডাটাবেসের পাহারা
     * শেষ কথা বলবে।
     *
     * ⚠️ হাতে লেখা নম্বরে এই হাঁটা **হয় না** — ওখানে ব্যবহারকারীকে
