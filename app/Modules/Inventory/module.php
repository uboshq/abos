...
return [
    ...
    'menu' => [
        ...
        'transactions' => [
            /*
             * ⭐ মাল গ্রহণ — মালিকের সীমানার টেবিল, ২৪ সেপ্টেম্বর ২০২৬।
             *
             * ── ⓘ কাগজটা এখনো Purchase-এ, দরজাটা এখানে ──────────────
             * মালিকের টেবিল বলে গ্রহণ **Inventory-র কাজ**, আর Purchase
             * কেবল সূত্র ও ট্রিগার দেবে। ⛔ কিন্তু মডেল, সেবা, রুট,
             * পর্দা আর তিন-মুখী মিলকরণের তিনটা সুইচ — সবই আজ Purchase-এ
             * বাঁধা, আর এক দিনে সরালে কয়েক দিন গ্রহণের পর্দা ভাঙা থাকত।
             *
             * ⭐ তাই দুই ধাপ, আর এটা প্রথমটা: **ব্যবহারকারীর কাছে
             * জিনিসটা আজ থেকেই Inventory-র**, অথচ একটা টেবিলও নড়ে না।
             * ⓘ ভিতরটা সরবে পরে, আর তখন এই সারিটার রুটই কেবল বদলাবে।
             *
             * ⚠️ চাবি ও সুইচ দুইটাই ক্রয়ের নিজেরটা — ⛔ নতুন চাবি
             * বানালে একই পর্দার দুইটা অনুমতি হত, আর নতুন সুইচ বানালে
             * একটা বন্ধ করে মানুষ অবাক হতেন যে পর্দাটা তবু খোলে।
             *
             * ⓘ ক্রয় মডিউল বন্ধ থাকলে সারিটা মেনুতেও আসে না —
             * [[MenuSwitches::itemIsOn()]] রুটের মডিউলটাও দেখে।
             */
            ['label' => 'inventory::menu.goods_receipt', 'icon' => 'inbox', 'from' => 'purchase',
                'route' => 'purchase.receipt.index', 'permission' => 'purchase.receipt.view',
             ...
             *
             * ⓘ গোনার পাশে: দুইটাই *"মাল দেখে সিদ্ধান্ত"*, আর
             * দুইটাতেই দেখা আর মেনে নেওয়া আলাদা হাতে।
             */
            ['label' => 'inventory::menu.quality', 'icon' => 'eye', 'route' => 'inventory.qc.index',
                'permission' => 'inventory.qc.view'],

            /*
             * ⭐ সিরিয়াল নম্বর — ২৪ সেপ্টেম্বর ২০২৬।
             *
             * ⓘ আইকন `search`, কারণ এই পর্দায় মানুষ আসেন **খুঁজতে**:
             * হাতে একটা নষ্ট টিভি, আর প্রশ্ন *"এটার ওয়ারেন্টি আছে?"*
             */
            ['label' => 'inventory::menu.serials', 'icon' => 'search', 'route' => 'inventory.serial.index',
        ...
        'reports' => [
             ...
             */
            /*
             * ⭐ কী কিনতে হবে — ২৪ সেপ্টেম্বর ২০২৬।
             *
             * ⓘ রিপোর্টের ভাগে, কারণ এটা পড়ার জিনিস: তালিকাটা দেখে
             * মানুষ ক্রয়ের পর্দায় যান। ⚠️ এখান থেকে সরাসরি অর্ডার
             * বানানো হয় না — সেটা ক্রয়ের সিদ্ধান্ত, মজুদের নয়
             * (মালিকের সীমানার টেবিল)।
             */
            ['label' => 'inventory::menu.replenishment', 'icon' => 'purchase', 'route' => 'inventory.report.show',
                'route_params' => ['slug' => 'replenishment'], 'permission' => 'inventory.report'],
    ...
    'permissions' => [
        ...
        /*
         * ⭐ পরিদর্শনের চাবি — ২৪ সেপ্টেম্বর ২০২৬।
         *
         * ── ⚠️ কেন রায়ের চাবিটা আলাদা ───────────────────────────
         * ⓘ কাগজ খোলা মানে *"এই মালটা দেখা দরকার"* — গুদামের
         * যে কেউ বলতে পারেন। ⛔ রায় মানে *"এই মাল নেওয়া হবে
         * না"*, আর ওটা সরবরাহকারীর সাথে তর্কে যায়, টাকার
         * হিসাবে যায়, আর মালটাকে বিক্রির বাইরে পাঠায়।
         *
         * ⚠️ মালিকের সিদ্ধান্ত (২৪ সেপ্টেম্বর): পরিদর্শক একটা
         * **আলাদা ভূমিকা**, আর পরীক্ষাটা পণ্য ধরে ধরে চালু —
         * সব পণ্যে নয়।
         */
        'inventory.qc.view',
        'inventory.qc.create',
        ...
        /*
         * ⭐ সিরিয়াল নম্বরের দুইটা চাবি — ২৪ সেপ্টেম্বর ২০২৬।
         *
         * ⓘ দেখা আর বসানো আলাদা: ⚠️ ওয়ারেন্টির দাবি নিয়ে আসা
         * মানুষটার পিসটা কাউন্টারের যে কেউ খুঁজে দেখতে পারেন,
         * ⛔ কিন্তু নম্বর **বসানো** মানে খাতায় একটা পিস জন্ম
         * দেওয়া, আর ঐ নম্বরই পরে ওয়ারেন্টির প্রমাণ।
         */
    ...
    'role_templates' => [
        'Warehouse' => [
            ...
            /*
             * ⓘ গুদামের লোক কাগজ খুলতে পারেন, রায় দিতে পারেন না
             * (`qc.decide` এখানে নেই, ইচ্ছাকৃতভাবে)।
             */
            'inventory.qc.view', 'inventory.qc.create',

            /* ⓘ গুদামের লোক পিস খোঁজেন ও বসান — দুইটাই তাঁর কাজ */
            'inventory.serial.view', 'inventory.serial.manage',
            'inventory.product.view',
        ],
    ...
    'doc_types' => [
        ...
        /*
         * ⭐ গুণমান পরিদর্শন — QC-2026-2027-0001, ২৪ সেপ্টেম্বর ২০২৬।
         *
         * ⓘ নিজের সিরিজ, কারণ সরবরাহকারীর সাথে তর্কের দিন কাগজটা
         * **নাম ধরে** খুঁজতে হয়। ⚠️ সিরিজ ছাড়া কাগজটার কোনো নাম
         * থাকত না, আর নাম ছাড়া কাগজ প্রমাণ হিসেবে চলে না।
         */
        'QC' => 'inventory::doc.quality_inspection',
     ...
     *
     * ⓘ মজুদ নড়ে, টাকা নয় — স্থানান্তর, সমন্বয়, গণনা; খাতার সারি পরে হিসাব থেকে আসে।
     *
