<?php

declare(strict_types=1);

return [
    'free_high' => 'Most free first',
    'name' => 'By name',
    'code' => 'By code',

    /* 'recent' lives in the block below, next to 'oldest' — all four lists
       use it (products, production, recipes, transfers), so it is declared once. */
    'available_low' => 'Least sellable first',
    'available_high' => 'Most sellable first',
    'floor_high' => 'Most on the floor first',
    'hold_high' => 'Most held first',
    'default_first' => 'Main store first',
    'on_the_way' => 'On the way first',
    'recent' => 'Newest first',
    'oldest' => 'Oldest first',

    'dish' => 'Dish name',
    'count_pending' => 'Unsettled first',
    'qc_pending' => 'Undecided first',
];
