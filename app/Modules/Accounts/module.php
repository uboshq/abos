 ...
 *
 * ── নামটা "Accounts & Finance" ছিল, আর সেটা ২৯ আগস্টেই পুরনো হয়ে গেছে ──
 * ওই দিন মালিক অর্থকে আলাদা মডিউল করেছেন
 * (`docs/Accounts ও Finance — দুই মডিউলের ভাগ.md`), আর সীমারেখাটা এক
 * বাক্যের: **এটা কি খাতায় দাখিলা লেখে, নাকি টাকার সিদ্ধান্ত নেয়?**
 * খাতা লেখে → হিসাব। সিদ্ধান্ত নেয় → অর্থ।
 *
 * `app/Modules/Finance` তখনই তৈরি হয়েছে আর নিজেকে "Finance / অর্থ" বলে,
 * কিন্তু এখানকার নামটা বদলানো হয়নি। ফল: **পর্দায় "হিসাব ও অর্থ" আর তার
 * ঠিক পাশেই "অর্থ"** — একই শব্দ দুই জায়গায়, আর ব্যবহারকারী বুঝতে পারেন
 * না কোনটায় কী।
 *
 * নামের ভুল নীরব: কোনো টেস্ট লাল হয় না, কিছুই ভাঙে না — শুধু প্রতিদিন
 * প্রত্যেকে ভুল দরজা খোঁজেন। ২ সেপ্টেম্বর ২০২৬-এ মালিক নিজে ধরিয়ে দেন।
 */
...
return [
     ...
     *
     * খাতা আগে, তারপর তার উপর দাঁড়ানো অর্থ।
     *
    ...
    'permissions' => [
        ...
        /*
         * ⛔ `accounts.voucher.approve` তোলা হলো — ২১ সেপ্টেম্বর ২০২৬।
         *
         * ⚠️ নামটা ছিল, কিন্তু কোথাও যাচাই হত না — অর্থাৎ ভূমিকার পর্দায়
         * **একটা অনুমোদনের অধিকার যেটা কিছুই আটকায় না**। ⛔ সেটা অকেজো
         * নয়, মিথ্যা: কেউ ঘরটা টিক দিয়ে ভাবতেন অনুমোদনের ক্ষমতা দেওয়া
         * হলো, অথচ কিছুই বদলাত না (abos-8b-র অডিট, চ৩)।
         *
         * ⓘ অনুমোদন সত্যিই আছে, কিন্তু অন্য পথে: নিচের `approvals`
         * ছকে ঘোষণা, আর কাজটা [[App\Core\Services\DocumentApproval]]-এর।
         * ⚠️ দুইটা ব্যবস্থা পাশাপাশি রাখলে একদিন কেউ ভুলটায় ভরসা করত।
         */
    ...
    'approvals' => [
         ...
         *
         * ⚠️ ── কেন এগুলো আগে ছিল না, আর তাতে কী হত ──────────────────
         * `VoucherApproval::stopping()` কাজের নাম হিসেবে ভাউচারের নিজের
         * ধরনটাই পাঠায় (`$voucher->type`) — অর্থাৎ পাঁচটাই ইঞ্জিনের
         * কাছে যায়। কিন্তু ঘোষিত ছিল কেবল `expense`, আর ছকের পর্দা
         * কেবল ঘোষিত কাজ দেখায়। ফল: **টাকা আদায় বা পরিশোধের ভাউচারে
         * অনুমোদন চাওয়ার কোনো পথই ছিল না** — কোড চাইত, দরজা ছিল না।
         *
         * ⓘ সারিগুলো কারো আজকের কাজ থামায় না: ছক না বসানো পর্যন্ত
         * `request()` `null` ফেরায় আর পোস্টিং আগের মতোই চলে
         * ([[VoucherApproval]])। এক ডিপোর "বড় পরিশোধ" আরেকটার রোজকার
         * কাজ, তাই সংখ্যাটা কোডে নয় — প্রতিটা কোম্পানি নিজে বসাবে।
         */
        'receipt' => 'accounts::approval.receipt',
     ...
     *
     * ⓘ খরচ · জমা · পরিশোধ · স্থানান্তর · জাবেদা — সবগুলোই খাতায় বসে; বছর বন্ধ করা ফেরানো যায় না।
     *
