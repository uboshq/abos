...
@else
    <form method="POST" action="{{ route('accounts.capital.post', $entry) }}"
        ...
        <select name="received_into_account_id" required
            ...
            <option value="" disabled selected>{{ __('accounts::field.landed_in') }}</option>
            @foreach ($accounts as $account)
                <option value="{{ $account->id }}">{{ $account->name() }}</option>
            @endforeach
        </select>

        <x-ui.button type="submit" tone="primary">{{ __('accounts::action.money_arrived') }}</x-ui.button>
    </form>
@endif
