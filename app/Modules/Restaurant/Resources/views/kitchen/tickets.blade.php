...
<x-layouts.app :menu="$menu">
    <x-slot:title>{{ __('restaurant::menu.kot') }}</x-slot:title>

    <x-slot:header>
        <x-ui.page-header :title="__('restaurant::menu.kot')"
                          :subtitle="__('inventory::message.tickets_note')" />
    </x-slot:header>

    {{--
        ⚠️ ব্যাখ্যাগুলো `x-data`-র **বাইরে**, আর কারণটা একটা লাইভ বাগ।

        ৩ সেপ্টেম্বর ২০২৬-এ এই মন্তব্যটা অ্যাট্রিবিউটের ভেতরে ছিল, আর
        তাতে বাংলা উদ্ধৃতিচিহ্ন হিসেবে **ডবল কোট** লেখা ছিল — অথচ পুরো
        `x-data="…"` নিজেই ডবল কোটে মোড়া। ব্রাউজার প্রথম `"`-এই
        অ্যাট্রিবিউটটা শেষ করে দিত, বাকিটা আবর্জনা হয়ে যেত, আর Alpine
        বলত *Invalid or unexpected token*।

        **ফল নীরব:** পাতাটা ২০০ দিত, দেখতে ঠিকই লাগত — কিন্তু
        `setInterval` কোনোদিন বসত না, সময়ের ঘর ফাঁকা থাকত, আর
        "সংযোগ নেই" ব্যাজটাও কখনো আসত না। **বাবুর্চি পুরনো পর্দার দিকে
        তাকিয়ে থাকতেন, নতুন অর্ডার আসত না, আর কোনো সতর্কবার্তাও নয়** —
        অথচ KOT পর্দার গোটা কাজই তো নিজে থেকে হালনাগাদ হওয়া।

        HP-র লাইভ যাচাইয়ে ধরা পড়ে: পাতা খুলে ২৫ সেকেন্ড অপেক্ষা করে
        নেটওয়ার্কে একটাও অনুরোধ যায়নি — অনুমান নয়, মাপা।

        ── কেন এখন এখানে ────────────────────────────────────────────
        HTML অ্যাট্রিবিউটের ভেতরে যা লেখা হয়, সেটা কোড নয়, **একটা
        স্ট্রিং** — আর স্ট্রিংয়ের ভেতরে ব্যাখ্যা লেখা মানে বিরামচিহ্নের
        উপর ভরসা করা। বাইরে থাকলে যা খুশি লেখা যায়।

        ── কেন দশ সেকেন্ড ───────────────────────────────────────────
        বোর্ডের চেয়ে ঘন, কারণ প্রশ্নটা আলাদা। আর কয় প্লেট হবে — সেটা
        মিনিটে বদলায়; নতুন অর্ডার এসেছে কি না — সেটা সেকেন্ডে।

        ── কেন গোটা পাতা নতুন করে আনা হয় ───────────────────────────
    ...
    <div x-data="{
        ...
        @if ($tickets->isEmpty())
            <x-ui.empty-state :message="__('restaurant::message.kitchen_quiet')" />
        @else
                        ...
                        <div class="mt-3 flex items-center gap-2">
                            @php
                                $next = match ($ticket->state) {
                                    \App\Modules\Restaurant\Models\KitchenTicket::PLACED
                                        => [\App\Modules\Restaurant\Models\KitchenTicket::COOKING, 'start_cooking'],
                                    \App\Modules\Restaurant\Models\KitchenTicket::COOKING
                                        => [\App\Modules\Restaurant\Models\KitchenTicket::READY, 'mark_ready'],
                                    default
                                        => [\App\Modules\Restaurant\Models\KitchenTicket::SERVED, 'mark_served'],
                                };
                            @endphp
                            ...
                            <span @class([
                                ...
                                'bg-(--color-badge-pending-bg) text-(--color-badge-pending-ink)'
                                    => $ticket->state === \App\Modules\Restaurant\Models\KitchenTicket::COOKING,
                                'bg-(--color-badge-success-bg) text-(--color-badge-success-ink)'
                                    => $ticket->state === \App\Modules\Restaurant\Models\KitchenTicket::READY,
                                'bg-(--color-surface-sunken) text-(--color-ink-muted)'
                                    => $ticket->state === \App\Modules\Restaurant\Models\KitchenTicket::PLACED,
                            ])>
                            ...
                            {{-- বড় বোতাম: রাঁধুনির হাত ভেজা, আর পর্দাটা
                                 হাতের নাগালেই থাকে। --}}
                            <form method="POST"
                                  action="{{ route('restaurant.kitchen.advance', $ticket) }}">
                                @csrf
