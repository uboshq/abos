...
final class PurchaseBillService
{
    ...
    public function create(array $data, array $lines): PurchaseBill
    {
        ...
        return DB::transaction(function () use ($data, $lines) {
            ...
            $bill = PurchaseBill::create([
                 ...
                 *
                 * ⭐ ভাড়াটা এখন সত্যিই ক্রয়মূল্যে ঢোকে — ২৪ সেপ্টেম্বর ২০২৬।
                 *
                 * ⓘ এখানে আগে লেখা ছিল *"ভাড়াটা কেবল রাখা হয়, এখনো
                 * ক্রয়মূল্যে ঢোকে না — ওটা আলাদা কাজ"*। ⭐ সেই আলাদা
                 * কাজটাই হয়ে গেছে: [[bringingInShares()]] টাকার
                 * অনুপাতে ভাগ করে স্তরে বসায়, আর [[postBringingIn()]]
                 * ঠিক ততটাই খতিয়ানে বসায়।
                 *
                 * ⚠️ নিশ্চিত করার সময় ভাগটা হয়, লেখার সময় নয় — খসড়া
                 * বিলে কোনো স্তরই বসে না, তাই ভাগ করার কিছু নেই।
                 */
    ...
    private function bringInDirectLines(PurchaseBill $bill): void
    {
        ...
        /*
         * ⭐ আনার খরচের ভাগ — লুপের **আগে**, একবারে গোটা তালিকা।
         *
         * ⚠️ সারি ধরে ধরে ভাগ করলে যোগফল ভাড়ার সমান হত না: প্রতিটা
         * ভাগে চার ঘরে কাটা পড়ে, আর বিশ সারিতে কয়েক পয়সা হারাত।
         * ⛔ ঐ পয়সাগুলো খতিয়ানে ঠিকই বসত (ওখানে গোটা ভাড়াটা), তাই
         * মজুদের খাতা আর হিসাবের খাতা আলাদা হয়ে যেত — নীরবে, আর
         * মাস শেষে।
         */
        $shares = $this->bringingInShares($bill, $direct);
        ...
        foreach ($direct as $line) {
             ...
             */
            $goodsValue = $this->goodsValueOf($line);
             ...
             */
            /*
             * ⭐ আনার খরচ মালের দামে — ২৪ সেপ্টেম্বর ২০২৬।
             *
             * ── ⛔ এই ফাঁকটা কোডেই লেখা ছিল ──────────────────────────
             * উপরে আমদানির ঘরগুলোর পাশে লেখা আছে: *"ভাড়াটা এখানে কেবল
             * রাখা হয়, এখনো ক্রয়মূল্যে ঢোকে না। ওটা আলাদা কাজ"*।
             * ⓘ এটাই সেই কাজ।
             *
             * ── ⚠️ কেন দরকার ───────────────────────────────────────
             * ⛔ ভাড়া বাদ দিয়ে ক্রয়মূল্য বসালে গুদামের মাল **সস্তা**
             * দেখাত, আর বেচার দিন মুনাফা ঠিক ততটাই **বেশি**। ⓘ
             * চট্টগ্রাম থেকে বস্তাপ্রতি বিশ টাকা ভাড়া লাগলে ঐ বিশ
             * টাকা মালেরই দাম।
             *
             * ── ⚠️ ভাগটা **টাকার অনুপাতে**, পরিমাণের নয় ──────────────
             * ⓘ একই ট্রাকে দশ বস্তা চাল আর দশ বাক্স ওষুধ এলে ভাড়া সমান
             * ভাগ করা যায় না: ওষুধের বাক্স হালকা আর দামি। ⛔ পরিমাণ
             * ধরে ভাগ করলে সস্তা মালের দাম লাফিয়ে উঠত।
             *
             * ⚠️ টাকার অনুপাতও নিখুঁত নয় (ন্যায্য ভাগ ওজন বা ঘনফুট ধরে),
             * ⓘ কিন্তু ওজন ABOS জানে না — আর **যে সংখ্যাটা আছে তার উপর
             * ভাগ করা** না-করার চেয়ে ভালো।
             */
            $share = $shares[$line->id] ?? '0';

            $this->costs->receiveWorth(
                product: $line->product,
                qty: (string) $line->qty,
                value: bcadd($goodsValue, $share, 4),
                sourceType: PurchaseBill::STOCK_SOURCE,
                sourceId: $bill->id,
                documentNo: $bill->document_no,
    ...
    /**
     * ⭐ এই বিলে মাল আনতে মোট কত খরচ হলো — ২৪ সেপ্টেম্বর ২০২৬।
     *
     * ── ⓘ কোন খরচগুলো "আনার খরচ" ─────────────────────────────────────
     * ভাড়া, আর আমদানিতে যা কিছু মাল হাতে পাওয়ার আগে দিতে হয়েছে।
     * ⚠️ আজ ABOS-এ শুধু `transport_cost` সংখ্যায় আছে; শুল্ক ও বন্দরের
     * ঘরগুলো এখনো কেবল **নম্বর** রাখে, টাকা নয়। ⓘ সেগুলোর টাকার ঘর
     * যেদিন আসবে, সেদিন এই একটা পদ্ধতিতেই যোগ হবে।
     *
     * ⛔ কর এখানে **নেই**, আর কোনোদিন থাকবে না: ভ্যাট ফেরতযোগ্য, ওটা
     * মালের দাম নয় — ⓘ একই কারণে নিচে `goodsValue`-তেও কর বাদ।
     */
    private function bringingInCost(PurchaseBill $bill): string
    {
        return (string) ($bill->transport_cost ?? '0');
    }

    /**
     * একটা সারির মালমূল্য — কর বাদ।
     *
     * ⛔ কর বাদ যায় কারণ ভ্যাট ফেরতযোগ্য, ওটা মালের দাম নয়। ⚠️ দুই
     * জায়গায় দুইভাবে গোনা হলে ([[bringInDirectLines()]] আর ভাগের
     * তালিকা) ভাগের অনুপাত আর স্তরের দাম আলাদা ভিত্তির উপর দাঁড়াত,
     * তাই সংখ্যাটার ঘর একটাই।
     */
    ...
    /**
     * ⭐ আনার খরচ কোন সারির ভাগে কতটা — ২৪ সেপ্টেম্বর ২০২৬।
     *
     * ── ⚠️ ভাগটা **টাকার অনুপাতে**, পরিমাণের নয় ──────────────────────
     * ⓘ একই ট্রাকে দশ বস্তা চাল আর দশ বাক্স ওষুধ এলে ভাড়া সমান ভাগ করা
     * যায় না: ওষুধের বাক্স হালকা আর দামি। ⛔ পরিমাণ ধরে ভাগ করলে সস্তা
     * মালের দাম লাফিয়ে উঠত, আর দামি মালেরটা প্রায় বদলাত না।
     *
     * ⚠️ টাকার অনুপাতও নিখুঁত নয় (ন্যায্য ভাগ ওজন বা ঘনফুট ধরে), ⓘ কিন্তু
     * ওজন ABOS জানে না — আর **যে সংখ্যাটা আছে তার উপর ভাগ করা**
     * না-করার চেয়ে ভালো।
     *
     * ── ⛔ শেষ সারিটা বাকিটুকু নেয় ─────────────────────────────────────
     * ⚠️ প্রতিটা ভাগ চার ঘরে নামানো হয়, তাই যোগফল ভাড়ার চেয়ে কয়েক পয়সা
     * কম বা বেশি হতে পারে। ⓘ খতিয়ানে বসে **এই তালিকার যোগফলই**
    ...
    {
        $cost = $this->bringingInCost($bill);

        $goodsTotal = $direct->reduce(
            fn (string $sum, PurchaseBillLine $line) => bcadd($sum, $this->goodsValueOf($line), 4),
            '0',
        );

        /*
         * ⚠️ মোট শূন্য হলে ভাগই হয় না — ⛔ নাহলে শূন্য দিয়ে ভাগ, আর
         * ডাটাবেজ নয়, PHP নিজেই থেমে যেত। ⓘ মোট শূন্য মানে সব সারিই
         * শূন্য টাকার, আর তখন ভাগ করার মতো কিছু নেইও।
         */
        if (bccomp($cost, '0', 4) <= 0 || bccomp($goodsTotal, '0', 4) <= 0 || $direct->isEmpty()) {
            return [];
        }
        ...
        foreach ($direct as $line) {
            // ⓘ ছয় ঘরে গুণ, তারপর চারে — সরাসরি চারে ভাগ করলে আগেই কাটা পড়ত
            $shares[$line->id] = bcdiv(bcmul($this->goodsValueOf($line), $cost, 6), $goodsTotal, 4);
            $given = bcadd($given, $shares[$line->id], 4);
        }

        $lastId = $direct->last()->id;
        $shares[$lastId] = bcadd($shares[$lastId], bcsub($cost, $given, 4), 4);

        return $shares;
    }

    /**
     * ⭐ ভাড়ার যতটা সত্যিই মালের দামে বসেছে — খতিয়ানের জন্য।
     *
     * ── ⚠️ কেন আলাদা করে গোনা, `transport_cost` সরাসরি নয় ─────────────
     * ⛔ বিলের সব সারির পেছনে চালান থাকলে এখানে কোনো স্তরই বসে না
     * ([[bringInDirectLines()]] তাদের ছোঁয় না — তাদের দাম আগেই বসেছে),
     * অথচ `transport_cost` ঘরে সংখ্যাটা থাকে। ⚠️ ওটা সরাসরি মজুদে ডেবিট
     * করলে হিসাবের খাতায় মজুদ বাড়ত আর গুদামের খাতায় কিছুই বাড়ত না।
     *
     * ⓘ তাই উত্তরটা আসে **যা সত্যিই বসেছে** তার যোগফল থেকে, আর তখন
     * দুই খাতার ফারাক গাণিতিকভাবেই অসম্ভব।
     */
    private function capitalisedBringingIn(PurchaseBill $bill): string
    {
        $direct = $bill->lines->filter(fn (PurchaseBillLine $line) => $line->receiptLine === null);

        return array_reduce(
            $this->bringingInShares($bill, $direct),
            fn (string $sum, string $share) => bcadd($sum, $share, 4),
            '0',
        );
    }

    /**
     * ⭐ আনার খরচটা খতিয়ানেও — ২৪ সেপ্টেম্বর ২০২৬।
     *
     * ── ⛔ এটা ছাড়া কাজটা সম্পূর্ণ অর্ধেক ─────────────────────────────
     * ভাড়া মালের দামে ঢোকানোর মানে গুদামের মাল **দামি** হলো। ⚠️ হিসাবের
     * খাতায় মজুদের ঘরটা না বাড়ালে ঐ দুই সংখ্যা আলাদা হয়ে যেত — আর
     * ফারাকটা ধরা পড়ত মাস শেষে, যখন কেউ আর মনে করতে পারে না কোন বিলে।
     *
     * ── ⓘ জোড়াটা নিজেই মেলে ──────────────────────────────────────────
     * ডেবিট মজুদ · ক্রেডিট পরিবহনের প্রদেয় — সমান, তাই ভাউচারের ভারসাম্য
     * বদলায় না। ⛔ এইজন্যই এটা `$difference` গোনার **পরে**: আগে বসালে
     * ডেবিটের যোগফল বেড়ে যেত, আর ভাড়ার সমান একটা "মূল্য-পার্থক্য" জন্মাত
     * — অর্থাৎ প্রতিটা ভাড়াওয়ালা বিল "মেলেনি" বলে দাগ খেত।
     *
     * ── ⚠️ ক্রেডিটটা সরবরাহকারীর ঘরে নয় ──────────────────────────────
     * ⓘ ভাড়ার টাকা গাড়িওয়ালার পাওনা, মিলের নয়; বিলের মোটেও ওটা নেই।
     * ⛔ সাধারণ প্রদেয়ে মিশিয়ে দিলে *"এই মাসে পরিবহনে কত বাকি"* প্রশ্নের
     * উত্তর আর বের করা যেত না — [[DeliveryChallanService::postTransportCost()]]
     * একই কারণে একই ঘর (২১১৬) ব্যবহার করে।
     *
     * ⓘ বাহক বাছা না থাকলে পক্ষ ছাড়াই বসে; [[CarrierAndLabourLedger]]
     * ঐ সারিগুলো *"পক্ষ বাছা নেই"* নামে দেখায়, লুকায় না।
     *
     * @param  list<array<string, mixed>>  $lines
     */
    private function postBringingIn(PurchaseBill $bill, array &$lines): void
    {
        ...
        /*
         * ⓘ ঘরটা না পেলে [[account()]] নিজেই বিলটা আটকে দেয়, খাতের কোড
         * নাম ধরে। ⛔ চুপচাপ ছেড়ে দিলে ভাড়াটা মালের দামে বসত আর খতিয়ানে
         * বসত না — ঠিক যে ফারাকটা ঠেকাতে এই পদ্ধতিটা লেখা।
         */
        $payable = $this->account(StandardChart::TRANSPORT_PAYABLE);

        $narration = __('purchase::message.bringing_in_cost', ['no' => $bill->document_no]);
    ...
    private function applySalesPrices(PurchaseBill $bill): void
    {
        foreach ($bill->lines as $line) {
             ...
             */
            $line->product->update(['purchase_price' => (string) $line->rate]);
     ...
     *
     * ── ⭐ ক্রেডিট **দুইটা** হতে পারে — ২৪ সেপ্টেম্বর ২০২৬ ─────────────
     * সরবরাহকারীর প্রদেয় (বিলের মোট), আর ভাড়া থাকলে পরিবহনের প্রদেয়
     * ([[postBringingIn()]])। ⚠️ এখানে আগে লেখা ছিল *"ক্রেডিটে একটাই"* —
     * ⓘ কথাটা সত্যি ছিল, আর ভাড়া মালের দামে ঢোকানোর দিন মিথ্যা হয়ে গেল।
     *
     * ⛔ ভাড়ার জোড়াটা নিজেই মেলে (ডেবিট মজুদ · ক্রেডিট পরিবহন), তাই
     * ভাউচারের ভারসাম্য বা `$difference` কোনোটাই বদলায় না।
     */
    private function postToLedger(PurchaseBill $bill): void
    {
        ...
        /*
         * ⭐ মিলকরণের ফলটা লেখা হয় — আটকানো হোক বা না হোক।
         *
         * ⛔ আগে ফলটা কোথাও থাকত না। সুইচ চালু থাকলে বিলটা আটকে যেত
         * (অর্থাৎ ঘটনাটা লেখাই হত না), আর সুইচ বন্ধ থাকলে বিলটা চুপচাপ
         * পাশ হয়ে যেত আর পার্থক্যটা কেবল মূল্য-পার্থক্য খাতে বসত।
         *
         * ⚠️ দুই অবস্থাতেই *"কোন বিলগুলো মেলেনি"* প্রশ্নের উত্তর ছিল
         * না — উত্তরটা বের করতে হিসাবের খাত ধরে উল্টোদিকে হাঁটতে হত।
         *
         * ⓘ লেখাটা আটকানোর **আগে**, ইচ্ছাকৃতভাবে: আটকানো মানে
         * ব্যতিক্রমটা ছুড়ে দেওয়া, আর তারপর আর কোনো লাইন চলে না।
         */
        ...
        $this->postBringingIn($bill, $lines);
    ...
    /**
     * ⭐ তিন-মুখী মিলকরণের ফল বিলের গায়ে বসানো — ২৪ সেপ্টেম্বর ২০২৬।
     *
     * ── ⓘ তিনটা কাগজ, আর তিনটাই আলাদা প্রশ্ন ─────────────────────────
     *   · **আদেশ** বলে কী কিনতে চেয়েছিলাম
     *   · **চালান** বলে কী সত্যিই এল
     *   · **বিল** বলে কী দাম চাওয়া হলো
     *
     * ⚠️ ফলটা চার রকম, আর চারটা চার জনের কাজ:
     *   `matched`   — তিনটাই মিলেছে; কারও কিছু করার নেই
     *   `partial`   — মিলেছে, কিন্তু কাগজের একটা অংশ এখনো আসেনি
     *   `mismatch`  — সংখ্যা মেলেনি, আর ফাঁকটা মাপা যায় (হিসাবরক্ষকের)
     *   `exception` — মেলানোই যায়নি: আদেশ নেই বা চালান নেই (ক্রয়ের)
     *
     * ⛔ `mismatch` আর `exception` এক করে ফেললে তালিকাটা মিথ্যা বলত:
     * দুই টাকার ফাঁকওয়ালা বিল আর আদেশবিহীন বিল একই সারিতে বসত, অথচ
     * দুইটা দুই বিভাগের কাজ।
     */
    private function recordMatch(PurchaseBill $bill, string $difference): void
    {
        $bill->forceFill([
            'match_state' => $this->matchStateFor($bill, $difference),
            'match_difference' => $difference,
        ])->save();
    }

    /**
     * ফলটা কী — আর ক্রমটাই এখানে আসল কথা।
     *
     * ⚠️ `exception` সবার আগে দেখা হয়। ⓘ যে বিলের কোনো চালানই নেই,
     * তার দামের ফাঁক মাপার কোনো মানে নেই — ⛔ ফাঁকটা তখন গোটা বিলের
     * সমান হত, আর সংখ্যাটা কাউকে কিছুই বলত না।
     */
    private function matchStateFor(PurchaseBill $bill, string $difference): string
    {
        $bill->loadMissing('lines');

        /*
         * ⛔ সংখ্যা না মিললে সেটাই আগে — বাকি সব প্রশ্ন তারপর।
         *
         * ⚠️ টাকার ফাঁকটাই একমাত্র জিনিস যা **মাপা যায়**, আর যেটা
         * মাপা যায় সেটাই আগে বলা উচিত।
         */
        if (bccomp($difference, '0', 4) !== 0) {
            return PurchaseBill::MATCH_MISMATCH;
        }

        $lines = $bill->lines->count();

        $withReceipt = $bill->lines->filter(
            fn ($line) => filled($line->purchase_receipt_line_id ?? null),
        )->count();

        $withOrder = $bill->lines->filter(
            fn ($line) => filled($line->purchase_order_line_id ?? null),
        )->count();

        /*
         * ⛔ প্রতিষ্ঠান বলেছে আদেশ ছাড়া মাল নেওয়া যাবে না, অথচ এই
         * বিলের পিছনে কোনো আদেশ নেই — ⓘ এটাই ব্যতিক্রম: মেলানোর মতো
         * তৃতীয় কাগজটাই নেই, অথচ থাকার কথা ছিল।
         *
         * ⚠️ সুইচটা বন্ধ থাকলে এটা ব্যতিক্রম **নয়** — তখন আদেশ ছাড়া
         * কেনা প্রতিষ্ঠানের নিজের নিয়ম, আর নিয়মকে ভুল বলা যায় না।
         */
        if ($withOrder === 0 && $this->settings->get('purchase.receipt_needs_order', false)) {
            return PurchaseBill::MATCH_EXCEPTION;
        }

        /*
         * ⚠️ **কিছু** সারি জোড়া, কিছু নয় — মিলকরণটা অসম্পূর্ণ।
         *
         * ── ⛔ প্রথম চেষ্টাটা এখানেই ভুল ছিল ──────────────────────────
         * আগে শর্তটা ছিল *"চালান আছে কিন্তু আদেশ নেই"*, আর তাতে
         * **প্রতিটা সরাসরি ক্রয়** আংশিক হয়ে যেত। ⓘ কিন্তু সেখানে
         * আদেশ না থাকাটাই স্বাভাবিক — কাগজ দুইটাই মেলে, আর দুই-মুখী
         * মিলকরণ একটা পূর্ণ মিলকরণ।
         *
         * ⭐ আংশিক মানে **ভিতরে অসমান**: একই বিলের কিছু সারি কাগজে
         * বাঁধা, কিছু নয় — আর ওটাই মানুষের দেখার জিনিস।
         */
        if (($withReceipt > 0 && $withReceipt < $lines)
            || ($withOrder > 0 && $withOrder < $lines)) {
            return PurchaseBill::MATCH_PARTIAL;
        }
