...
final class DirectPurchaseService
{
    ...
    public function complete(array $data, array $lines, array $gifts = []): array
    {
         ...
         * মালিকের অভিযোগ: *"বিল create করার পর খুঁজে পাওয়া যায় না কেন?"*
         * পর্দা বলত *"This has gone for approval"*, অথচ তালিকায় কিছু নেই,
         * অনুমোদনের ইনবক্সেও কিছু নেই।
         *
         * ⓘ [[PurchaseBillService::confirm()]]-এ `assertClear()` ইচ্ছে
         * করেই লেনদেনের **বাইরে** — সেখানকার মন্তব্যেই লেখা *"অপেক্ষা
         * করা মানে কিছুই না বসা, আধা-বসা নয়"*। ⭐ ওটা ঠিক ছিল।
         *
         * ⚠️ কিন্তু আগে এই পুরো মেথডটা একটাই `DB::transaction`-এ মোড়া
         * ছিল, আর তাতে ভেতরের সাবধানতাটা অর্থহীন হয়ে যেত: ব্যতিক্রম
         * ছোঁড়ার সাথে সাথে বাইরের লেনদেন রোলব্যাক করত, আর **ইঞ্জিন
         * এইমাত্র যে অনুমোদনের অনুরোধটা লিখেছিল সেটাও মুছে যেত**।
         *
         * ⛔ ফল: কাগজ নেই, অনুরোধ নেই, কেবল একটা বাক্য যা মিথ্যা বলে —
         * আর প্রতিবার চেষ্টায় হুবহু একই, বেরোনোর পথ ছাড়া।
         *
         * ── ⓘ এখন সীমানা তিনটা, আর প্রতিটার নিজের কারণ ──────────────
         *   ১. কাগজ ও তার সারি — নিজের লেনদেনে। আটকালে **খসড়া হয়ে
         *      থেকে যায়**, তাই খুঁজে পাওয়া যায় আর সই হলে নিশ্চিত করা যায়।
         *   ২. সই — কোনো লেনদেনে নয়। `confirm()` নিজের ভেতরে মজুদ আর
         *      খতিয়ান একসাথেই বসায়, তাই ওখানে আণবিকতা অক্ষত।
         *   ৩. মাল নড়া ও টাকা — নিজের লেনদেনে।
         *
         * ⚠️ যা হারাল: আগে ধাপ ৩ ভাঙলে বিলটাও উবে যেত। ⓘ এখন বিলটা
         * নিশ্চিত থেকে যায় আর উপহার বা টাকা বসে না। ⛔ কিন্তু বিনিময়ে
        ...
        $bill = DB::transaction(function () use ($data, $lines, $gifts) {
            ...
            /*
             * ⭐ উপহারের **সারিগুলো** কাগজের সাথেই — ওরা নথি, ঘটনা নয়।
             *
             * ⓘ আগে এটা `confirm()`-এর পরে ছিল, আর কারণটা ছিল মালের
             * চলাচল। ⚠️ কিন্তু `writeGifts()` এক গ্রাম মালও নাড়ে না,
             * সে কেবল সারি লেখে — নাড়ে [[bringInGifts]], আর সেটা
             * নিচেই, নিশ্চিত হওয়ার পরে।
             *
             * ⛔ সারিগুলো পরে লিখলে সই আটকানো খসড়াটা **উপহার ছাড়া**
             * পড়ে থাকত, আর সই হওয়ার পর কেউ মনে করতে পারতেন না লরিতে
             * আর কী এসেছিল।
             */
    ...
    private function billLines(array $lines): array
    {
        return array_map(fn (array $line) => [
             ...
             * ⚠️ এই লাইনটা ছাড়া উপরের ছাঁকনি ওটা গিলে ফেলত, আর পর্দায়
             * "১ কার্টন ফ্রি" বেছেও খাতায় পিস বসত — নীরবে।
             */
            'free_unit_id' => $line['free_unit_id'] ?? null,

            'discount' => (string) ($line['discount'] ?? '0'),

            /*
             * ⛔ ভ্যাটের ঘরটা **null থাকলে null-ই যায়** — আগে এখানে
             * `?? '0'` লেখা ছিল, আর ওটাই একটা নীরব বাগ ছিল।
             *
             * ── কী ভাঙা ছিল ─────────────────────────────────────────
             * [[CalculatesLineTotals::lineFigures()]] তিনটা পথ চেনে:
             *
             *   null / ''  → পণ্যের নিজের হার থেকে গোনা
             *   অঙ্ক        → সেটাই, আর পার্থক্যটা `tax_variance`-এ
             *   '0'        → ভ্যাট নেই
             *
             * ⚠️ `?? '0'` তৃতীয় পথটাকে প্রথমটার জায়গায় বসিয়ে দিত, তাই
             * **সরাসরি ক্রয়ে পণ্যের নিজের ভ্যাটের হার কখনো বসতই না** —
             * অথচ ক্রয় বিলের পর্দায় বসত ([[PurchaseBillService]] সেখানে
             * `?? null` পড়ে)। ⛔ একই পণ্য, দুই পর্দা, দুই রকম ভ্যাট।
             *
             * ⓘ মালিকের নকশার ভ্যাট-ড্রপডাউনটা ঠিক এই তিনটা পথেরই মুখ,
             * আর "পণ্য অনুযায়ী" বাছলে পর্দা ঘরটা **পাঠায় না** — তাই
             * এই লাইনটা ছাড়া ওই ধরনটা কোনোদিন কাজ করত না।
             */
    ...
    private function bringInGifts(PurchaseBill $bill): void
    {
        ...
        foreach ($bill->giftLines as $gift) {
            $this->stock->move(
                ...
                /*
                 * ⚠️ `free` নয়, `unplacedFree` — ৫ সেপ্টেম্বর ২০২৬।
                 *
                 * ── কী ভাঙা ছিল ─────────────────────────────────────
                 * উপহারের কার্টনটা সরাসরি **তাকে** বসত, অথচ একই
                 * গাড়ির কেনা মাল আর ফ্রি পরিমাণ দুইটাই অপেক্ষার ঘরে
                 * যায় ([[PurchaseBillService:460]])।
                 *
                 * ⛔ **একই লরির মাল, দুই নিয়ম।** ফল দুইটা, দুইটাই খারাপ:
                 *
                 *   বসানোর পর্দায় উপহারটা কোনোদিন আসত না — গুদামের
                 *   লোক জানতেনই না ওটা এসেছে
                 *
                 *   আর সে সাথে সাথেই বিক্রয়যোগ্য — অর্থাৎ মালিকের নিয়ম
                 *   ("বসানোর আগে বিক্রি নয়") উপহারের বেলায় খাটত না
                 *
                 * ⓘ ধরা পড়েছে সহকর্মীর মাপায়, কোড পড়ে — কোনো টেস্ট
                 * এটা ধরেনি, কারণ প্রতিটা পরীক্ষা নিজের দিকটাই মাপে
                 * আর দুইটার **তুলনা** কেউ করেনি।
                 */
                unplacedFree: (string) $gift->qty,
            );
        }
    }
    ...
    public function stockPanel(Product $product, ?Warehouse $warehouse = null): array
    {
        ...
        return [
            ...
            /*
             * ⭐ পণ্যের নিজের ভ্যাটের হার — পর্দার "Per product" ধরনটার জন্য।
             *
             * ── কেন হার, অঙ্ক নয় ────────────────────────────────────
             * অঙ্কটা লাইনের নেট মূল্যের উপর বসে, আর নেট মূল্য প্রতিটা
             * কি-স্ট্রোকে বদলায়। হার পাঠালে পর্দা নিজেই কষতে পারে, আর
             * প্রতিটা অক্ষরে সার্ভারে যেতে হয় না।
             *
             * ⚠️ নিয়মটা [[CalculatesLineTotals::lineFigures()]]-এর হুবহু
             * — ভিতরের ভ্যাটে মোট বাড়ে না, তাই `is_inclusive`-টাও যায়।
             * ⓘ দুই জায়গায় দুই নিয়ম হলে পর্দা এক সংখ্যা দেখাত আর খাতায়
             * আরেকটা বসত, আর কেউ বলতে পারত না কোনটা সত্যি।
             */
            'tax_rate' => (float) ($product->tax?->rate ?? 0),
            'tax_inclusive' => (bool) ($product->tax?->is_inclusive ?? false),
        ];
    }
