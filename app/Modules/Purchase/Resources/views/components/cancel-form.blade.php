...
<details class="rounded-(--radius-card) border border-(--color-border) bg-(--color-surface-card) p-4">
    <summary class="cursor-pointer text-sm font-medium">{{ __('purchase::action.cancel_document') }}</summary>

    <form method="POST" action="{{ $action }}" class="mt-3 space-y-3">
        @csrf
        <x-ui.field name="reason" :label="__('purchase::message.cancel_reason')" required />
        <x-ui.button type="submit" tone="danger">{{ __('purchase::action.cancel_document') }}</x-ui.button>
    </form>
</details>
