...
final class CredentialCheck
{
    ...
    /**
     * ⛔ `name` আর মেলানো হয় না — ১৪ সেপ্টেম্বর ২০২৬।
     *
     * ⓘ এর আগে এখানে লেখা ছিল *"ওই ঘর যেদিন যোগ হবে, সেদিন এখানে
     * একটা লাইন"* — মোবাইলের ঘরটার কথা, যেটা ২ সেপ্টেম্বর টেবিলে ছিল
     * না। ⭐ ঘরটা আজ যোগ হয়েছে, আর লাইনটা এই নিচেই।
     *
     * ── কী ভুল ছিল ───────────────────────────────────────────────────
     * ⚠️ `users.name`-এ unique সূচক **নেই** (মেপে দেখা: আছে কেবল
     * `users_email_unique` ও `users_public_id_unique`), আর নিজের নাম
     * প্রোফাইল পাতা থেকে যে কেউ নিজেই বদলাতে পারেন।
     *
     * ⛔ তাই একজন নিজের নামের ঘরে আরেকজনের **ইমেইল** বসিয়ে দিলে ঐ
     * ঠিকানা দিয়ে খোঁজায় দুইটা সারি মিলত, আর `first()` কোনটা ফেরাবে
     * তার কোনো নিয়ম নেই। ⓘ যাঁর ইমেইল, তাঁর পাসওয়ার্ড তখন ভুল সারির
     * বিপরীতে মেলানো হত — অর্থাৎ **তিনি নিজের অ্যাকাউন্টে ঢুকতে
     * পারতেন না**, আর কারণটা তাঁর পর্দায় লেখা থাকত "ভুল পাসওয়ার্ড"।
     *
     * ⭐ এখন পরিচয় তিনটা, আর তিনটাই **unique বা নিজস্ব**:
     *
     *     login_id  → নিজের বাছা আইডি, ডাটাবেজে unique
     *     email     → unique
     *     mobile    → নিজের নম্বর
     *
     * ⓘ আর এতে লগইনের ঘরের লেবেলটাও সত্যি হলো: *"ব্যবহারকারীর নাম,
     * ইমেইল বা মোবাইল"* — মোবাইল এতদিন লেখা ছিল, মেলানো হত না।
     *
     * ── ⚠️ মোবাইল unique নয়, আর সেটা জেনেবুঝে ───────────────────────
     * একই নম্বর দুইজনের হতে পারে (স্বামী-স্ত্রী, বাবা-ছেলে একই দোকানে)।
     * ⛔ তাই নম্বর দিয়ে খুঁজলে দুইটা সারি মিলতে পারে — এবং তখন
     * **কাউকেই ফেরানো হয় না**, নিচের দ্বিতীয় প্রশ্নটা দেখুন। ⓘ চুপ করে
     * প্রথমটা ফেরানো মানে ঠিক আজকের ভুলটাই নতুন ঘরে ফিরিয়ে আনা।
     *
     * ── ⓘ `whereNotNull` কেন ─────────────────────────────────────────
     * ⚠️ যাঁদের `login_id` বা `mobile` খালি, তাঁরা সবাই NULL। খালি
     * পরিচয় দিয়ে খুঁজলে যাতে তাঁরা কেউ না মেলেন — যদিও খালি স্ট্রিং
     * ভ্যালিডেশনেই আটকায়, এই দেয়ালটা সস্তা।
     *
     * ── পুরনো নোট, এখনো খাটে ─────────────────────────────────────────
     * বন্ধনীটা জরুরি: `User`-এ `SoftDeletes` আছে, তাই Eloquent নিজে
     * `deleted_at is null` জোড়ে। ⛔ বন্ধনী ছাড়া `AND`-এর অগ্রাধিকার
     * বেশি হয়ে যেত আর নরম-মোছা ব্যবহারকারীও ফিরে আসতেন।
     */
    private function findUser(string $identifier): ?User
    {
        ...
        /*
         * ⛔ একের বেশি মিললে কাউকেই নয়।
         *
         * ⚠️ এটাই আজকের ভুলটার আসল শিক্ষা: অস্পষ্টতার মুখে চুপ করে
         * একটা বেছে নেওয়া মানে ভুলটা লুকিয়ে ফেলা। ⓘ কেউ তাঁর নম্বর
         * দিয়ে ঢুকতে না পারলে তিনি ইমেইল বা আইডি দিয়ে ঢুকবেন, আর
         * ব্যবস্থাটা কখনো ভুল মানুষকে ভেতরে ঢোকাবে না।
         */
