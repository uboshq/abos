 ...
 *
 * ── শ্রোতারা একই অনুরোধে চলে — worker আছে, কিন্তু কেউ কিউতে কাজ দেয় না ──
 * ⓘ শ্রোতা একই অনুরোধে, একই প্রক্রিয়ায় চলে। কারণ ছিল: কিউ মানে আরেকটা
 * চলমান অংশ (worker, supervisor, ব্যর্থ কাজের টেবিল), যেটা অফিসের একটা
 * মেশিনে একদিন চুপচাপ বন্ধ হয়ে থাকত, আর কেউ টের পেত না।
 *
 * ⚠️ ২৬ সেপ্টেম্বর ২০২৬-এর মাপ: এখানে আগে লেখা ছিল *"এখনো কিউ নেই"*,
 * আর সেটা আর সত্য নয়। ১৭ সেপ্টেম্বর থেকে `routes/console.php`-এ
 * `queue:work --stop-when-empty --max-time=50 --tries=3` প্রতি মিনিটে
 * চলে — কোনো supervisor ছাড়া, cron-এর `schedule:run` থেকে। ⛔ কিন্তু
 * কোনো শ্রোতা বা কাজ `ShouldQueue` বাস্তবায়ন করে না, তাই worker
 * প্রতিবার খালি হাতে ফেরে।
 *
 * ⓘ যেদিন সত্যিই লাগবে (SMS, ইমেইল, বড় রিপোর্ট), শ্রোতাটা ShouldQueue
 * বাস্তবায়ন করলেই worker ধরবে — ঘোষণার জায়গা বদলাবে না। ⚠️ ব্যর্থ কাজ
 * যায় `failed_jobs`-এ, আর ঐ টেবিল কেউ খোলে না; তাই টাকা বা বাইরের
 * সংযোগের কাজ কিউতে দেওয়ার আগে `docs/Plan — Integration Platform ও
 * Auto Approval.md` §গ১ দেখুন।
 */
class EventRegistry
{
