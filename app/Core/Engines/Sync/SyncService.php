...
final class SyncService
{
    ...
    private function applyOne(
        ...
        /*
         * ⚠️ পড়ার চেয়ে লেখার পাহারা কম হতে পারে না।
         *
         * চাবির যাচাইটা এতদিন কেবল `pull()`-এ ছিল — অর্থাৎ গ্রাহকের নাম
         * *পড়তে* অনুমতি লাগত, কিন্তু সেই নামে একটা আদায় *বসাতে* লাগত না।
         * উল্টো, আর কোনো যুক্তিতে দাঁড়ায় না। তাই push-ও একই চাবি চায়।
         *
         * ── কেন এখানে REJECTED, `pull()`-এর মতো চুপচাপ বাদ নয় ──────────
         * `pull()`-এ অনুমতি না থাকলে সারিটা চুপচাপ বাদ পড়ে — "আমার জন্য নয়",
         * আর সেটা ঠিক। কিন্তু push-এ ঠিক সেই যুক্তিই উল্টো ফল দেয়: চুপচাপ
         * ফেলে দিলে ফোন ভাবত সিঙ্ক হয়ে গেছে, সেলসম্যান টাকা তুলে দোকান থেকে
         * বেরিয়ে যেতেন, খাতায় কিছুই বসত না, আর তিন দিন পরে হিসাব মিলত না।
         * তাই সৎ প্রত্যাখ্যান — বার্তাসহ, ফোনের পর্দায় দেখা যায়।
         *
         * ⓘ ঘোষিত অথচ অপ্রযুক্ত চাবি সবচেয়ে বিপজ্জনক: পড়ে মনে হয় পাহারা
         * আছে। হ্যান্ডলারগুলো বহুদিন `requiredPermission()` লিখে রেখেছিল,
         * অথচ push সেটা কোনোদিন পড়েনি।
         */
    ...
    /**
     * ⚠️ হ্যান্ডলারদের যে সময়টা দেওয়া হয় — ওয়াটারমার্ক নয়, তার **এক
     * মিনিট আগে**।
     *
     * ── কেন, আর এটা না থাকলে কী নীরবে ভাঙত ──────────────────────────
     * ওয়াটারমার্ককে একটা ছুরির ধার ধরলে দুইটা জিনিস চিরতরে হারায়, আর
     * **দুইটাই কোনো ভুল না দেখিয়ে**।
     *
     * **এক — লেনদেনের দৃশ্যমানতা।** `created_at` বসে INSERT চলার
     * মুহূর্তে, কিন্তু সারিটা অন্য সংযোগের চোখে পড়ে **COMMIT-এর পরে**।
     * একটা বিল পোস্ট হতে ৪০০ মিলিসেকেন্ড লাগলে তার খতিয়ান-সারিগুলোয়
     * সময় বসে T, আর দেখা যায় T+০.৪-এ। সিঙ্ক যদি T+০.২-তে পড়ে, সে
     * কিছুই দেখে না, কার্সারটা T পেরিয়ে যায়, আর **ওই সারিগুলো
     * কোনোদিন পাঠানো হয় না**। কোনো ত্রুটি নয়, ফোনে দেখার মতো কোনো
     * ফাঁকও নয় — শুধু ওই দোকানের বকেয়া ওই যন্ত্রে চিরকাল ভুল, আর
     * নিজে থেকে ঠিক হয় কেবল যদি দোকানটা আবার লেনদেন করে।
     *
     * **দুই — একই সেকেন্ডের সীমানা।** `created_at` সেকেন্ড-নিখুঁত।
     * একই সেকেন্ডে তিনটা সারি বসলে আর কার্সার `> $last` হলে যেগুলো
     * পড়া হয়েছে সেগুলো থাকে, বাকিগুলো হারায়।
     *
     * ── কেন `MAX(id)` দিয়ে সারানো যায় না ────────────────────────────
     * auto-increment মান দেওয়া হয় insert-এর সময়, আর commit হতে পারে
     * এলোমেলো ক্রমে — একই গর্ত, অন্য চেহারায়।
     *
     * ── সমাধানটা ইতিমধ্যেই তৈরি ছিল ─────────────────────────────────
     * এক মিনিট পিছিয়ে পড়া হয়, আর ওই ওভারল্যাপটা পাঠিয়ে দেওয়া হয়।
     * পুনরাবৃত্তি ক্ষতিকর নয়: ফোনের ক্যাশ `entityType:entityId` ধরে
     * বসায়, তাই একই রেকর্ড দ্বিতীয়বার এলে একই ঘরে একই মান বসে।
     * পুশের দিকে দুইবার-বসা ঠেকানোর যে ব্যবস্থাটা আছে, এটা তার
     * পড়ার-দিকের যমজ — একই সিদ্ধান্ত, দ্বিতীয়বার মূল্য দিচ্ছে।
     *
     * এক মিনিট উদার: ABOS-এর দীর্ঘতম পোস্টিং লেনদেন এর ধারেকাছেও নয়।
     * দাম প্রতি সিঙ্কে গুটিকয় বাড়তি সারি।
     *
     * ⚠️ এটা এখানে, প্রতিটা হ্যান্ডলারে নয় — কারণ হ্যান্ডলারে রাখলে
     * দশম হ্যান্ডলারটা লিখবেন এমন কেউ যিনি আগেরগুলো নকল করছেন, আর
     * তিনি বাদ দিলে **কিছুই লাল হত না**।
     */
    private const OVERLAP_SECONDS = 60;

    private function cursorFor(string $deviceId, string $module): ?Carbon
    {
        return $this->lastSync($deviceId, $module)?->copy()->subSeconds(self::OVERLAP_SECONDS);
    }
