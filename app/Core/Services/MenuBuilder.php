            ...
            foreach ($module->menu as $group => $items) {
                ...
                foreach ($items as $item) {
                    ...
                    $visible[] = [
                        ...
                        /*
                         * সারির নিজের আইকন — মালিকের নির্দেশ, ১৩ সেপ্টেম্বর।
                         *
                         * ⓘ আগে এখানে আইকনটা আসতই না, তাই পর্দা বাধ্য হয়ে
                         * **গ্রুপের** আইকন বসাত — আর তাতে একটা গ্রুপের সব
                         * সারি একই চিহ্ন পেত ("লেনদেন"-এর আটটাই ⇄)।
                         *
                         * ⚠️ নামটা এখানে অনুবাদ করা হয় না, কারণ এটা লেখা
                         * নয় — একটা চাবি, আর `x-ui.icon` ওটা দিয়েই আঁকে।
                         * অচেনা নাম দিলে সে চুপ করে কিছুই আঁকে না।
                         */
                        'icon' => $item['icon'] ?? null,

                        /*
                         * ⓘ দলের ভেতরের ভাঁজ — ১৯ সেপ্টেম্বর ২০২৬, মালিক: ভাউচার
                         * তালিকা, আদায়, পরিশোধ, খরচ, জাবেদা, কন্ট্রা *"eigulo mile
                         * ekta group korlei hoy"*। একই `cluster` নামের সারিগুলো
                         * উপরের সারিতে এক ড্রপডাউনে বসে ([[shell.modulebar]])।
                         */
                        'cluster' => $item['cluster'] ?? null,

                        /*
                         * ⭐ সারিটা বারে বসবে, ভাঁজের ভিতরে নয় — ২৪ সেপ্টেম্বর ২০২৬।
                         *
                         * ── ⓘ মালিকের কথা ──────────────────────────────────
                         * *"Control Panel setings er baire thakbe bare"*, আর
                         * *"master group bad diye direct bare menu bosaw"*।
                         *
                         * ── ⚠️ কেন সারি-প্রতি, দল-প্রতি নয় ─────────────────
                         * ⓘ [[shell.modulebar]]-এ `$neverGrouped` আগে থেকেই আছে,
                         * কিন্তু ওটা **গোটা দলটাকে** খোলে। ⛔ সিস্টেম প্রশাসনের
                         * সেটিংস দলে দশটা সারি — কন্ট্রোল প্যানেল বারে আনতে গিয়ে
                         * বাকি নয়টাও বারে উঠে যেত, আর বারটা পড়াই যেত না।
                         *
                         * ⚠️ আর ওটা সব মডিউলের জন্য এক তালিকা: `master` ঢোকালে
                         * হিসাব মডিউলের ঊনত্রিশটা সারিও খুলে যেত।
                         */
                        'loose' => ($item['loose'] ?? false) === true,
                        'route' => $item['route'],
