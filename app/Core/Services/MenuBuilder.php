...
final class MenuBuilder
{
        ...
        foreach ($this->registry->all() as $module) {
            ...
            foreach ($module->menu as $group => $items) {
                ...
                foreach ($items as $item) {
                    ...
                    /*
                     * রুটটা সত্যিই নিবন্ধিত কি না — ⛔ আর এটা `planned`-এর
                     * বিকল্প নয়, তার বাইরের একটা অবস্থা।
                     *
                     * ── কী ঘটেছিল, ১৩ সেপ্টেম্বর ২০২৬ ────────────────────
                     * একটা মডিউলে মেনুর সারিটা বসেছে, কিন্তু রুটগুলো তখনো
                     * নিবন্ধিত হয়নি। ফল: `route()` ছুঁড়ে দিয়েছে, মেনু
                     * নেমে গেছে, আর **যে পর্দাই মেনু আঁকে সেটাই ৫০০** —
                     * পুরো সুইটের ৮০টা ব্যর্থতার ৬৫টা, একটা সারি থেকে।
                     *
                     * ⚠️ `planned` পতাকাটা এই অবস্থাটা ঢাকে না, কারণ ওটা
                     * **হাতে রাখা**: ওটা বলে "ইচ্ছাকৃতভাবে এখনো বানানো
                     * হয়নি"। কিন্তু রুট থাকার **কথা** অথচ নেই — বাসি
                     * রুট-ক্যাশ, নামের টাইপো, অর্ধেক যোগ করা তালিকা — এই
                     * অবস্থায় কেউ পতাকাটা তোলে না, আর তোলার কথাও নয়।
                     *
                     * ⭐ প্যাটার্নটা রিপোতে আগে থেকেই আছে —
                     * [[FinanceDashboard]] লেখে `Route::has(...) ? route(...) : null`।
                     * ড্যাশবোর্ড নিজেকে রক্ষা করত, মেনু করত না; অথচ মেনু
                     * প্রতিটা পর্দায় আঁকা হয়, তাই সেখানে দামটা ৬৫ গুণ।
                     *
                     * ⓘ সারিটা তখন তালিকায় থাকে কিন্তু নিষ্ক্রিয় দেখায় —
                     * নীরবে উধাও হয় না, কারণ উধাও হলে কেউ খুঁজতই না।
                     */
                    $reachable = Route::has($item['route']);
                    ...
                    $visible[] = [
                        ...
                        'url' => $planned || ! $reachable ? null : route($item['route'], $params),
                        'planned' => $planned,
                        'active' => ! $planned && $reachable
                            && $this->onThisScreen($item['route'], $exact)
                            && $this->paramsMatch($params),

                        /*
                         * ⭐ সারিটা কত কাজ পড়ে আছে তা বলতে পারে।
                         *
                         * ── মালিকের কথা, ৫ সেপ্টেম্বর ২০২৬ ──────────────
                         * সরাসরি ক্রয়ের পর্দায় একটা **স্থায়ী** নোটিশ
                         * বসানো ছিল — *"মাল ঢোকে বসানো হয়নি অবস্থায়…"*।
                         * তিনি ধরলেন: *"এটার জন্য Notice board আছে কেন?
                         * All time Notice চলছে কী জন্য?"*
                         *
                         * ⚠️ কথাটা ঠিক: যে লেখা রোজ ওঠে তা কেউ পড়ে না,
                         * আর তখন **একই জায়গার আসল সতর্কতাগুলোও** পড়া বন্ধ
                         * হয়ে যায়। ⓘ কাউন্টারের লোক দিনে পঞ্চাশবার ঐ
                         * পর্দা খোলেন।
                         *
                         * ⭐ **সংখ্যা তথ্য, বাক্য উপদেশ।** "৩" দেখলে মানুষ
                         * ক্লিক করেন; একটা উপদেশ পড়ে কেউ কিছু করেন না।
                         *
                         * ── ⚠️ কোরে কোনো মডিউলের নাম নেই ────────────────
                         * সারিটা নিজেই বলে সংখ্যাটা কে গোনে
                         * (`'count' => SomeService::class`), আর কোর কেবল
                         * ডাকে। ⛔ নাহলে §১৯.৭ ভাঙত, আর অনুমোদন-অপেক্ষা বা
                         * চেকের মেয়াদ যোগ করতে এই ফাইলটা আবার খুলতে হত।
                         *
                         * ⓘ শূন্য হলে ব্যাজ নেই — একটা "০" ব্যাজ প্রতিদিন
                         * চোখে পড়ে আর কিছুই বলে না, অর্থাৎ ঐ স্থায়ী
                         * নোটিশেরই আরেক রূপ।
                         */
                        'badge' => $planned ? null : $this->badge($item),
                    ];
                }
     ...
     *
     * ⚠️ **আজ কোনো মডিউল `nav.under` ঘোষণা করে না — এই পদ্ধতিটা কিছুই করে না।**
     *
     * ── তাহলে রাখা কেন, ৪ সেপ্টেম্বর ২০২৬ ────────────────────────────
     * সকালে গ্রাহক বিক্রয়ের ভেতরে আর সরবরাহকারী ক্রয়ের ভেতরে বসানো
     * হয়েছিল, মালিকের নির্দেশে; বিকেলে তিনি সিদ্ধান্ত ফিরিয়ে নিয়েছেন।
     *
     * যন্ত্রটা রয়ে গেল দুইটা কারণে। এক, সিদ্ধান্তটা রুচির — কাল আবার
     * বদলাতে পারে, আর তখন এটা এক লাইনের কাজ। দুই, এর সাথে আসা `codes`
     * কী-টা **সত্যিকারের একটা বাগ সারিয়েছে**: শেলের আটটা চেহারা "আমি
     * কোথায় আছি" বের করত রুটের উপসর্গ দেখে, আর সেই নিয়মটা আটবার কপি
     * করা ছিল। ⓘ `codes` না থাকলে নবম চেহারাটা আবার নিজের কপি লিখত।
     *
     * ⛔ তাই এটা মৃত কোড নয়, **অব্যবহৃত পথ** — আর পার্থক্যটা এখানে
     * লেখা থাকল, যাতে কেউ ছয় মাস পরে "কেউ ডাকে না" দেখে মুছে ফেলার
     * আগে জানে কী সাথে যাবে।
     *
    ...
    /**
     * সারির ব্যাজের সংখ্যা — যে সেবাটা গোনে সে-ই দেয়।
     *
     * ── ⚠️ গোনা ব্যর্থ হলে মেনু পড়ে যাবে না ─────────────────────────
     * মেনু **প্রতিটা পাতায়** আঁকা হয়। ⛔ কোনো একটা গণনার সেবা ব্যতিক্রম
     * ছুড়লে গোটা অ্যাপটাই অচল হত — একটা সংখ্যার দামে। ⓘ তাই ব্যর্থ
     * হলে ব্যাজ থাকে না, বাকিটা আগের মতোই চলে।
     *
     * ⚠️ এটা "ভুল লুকানো" নয়: সংখ্যাটা সাজসজ্জা, আর তার জন্য দরজা বন্ধ
     * করা অসামঞ্জস্যপূর্ণ। যে সেবাটা ভাঙা তার নিজের পরীক্ষা তাকে ধরবে।
     *
     * @param  array<string, mixed>  $item
     */
    private function badge(array $item): ?int
    {
        $counter = $item['count'] ?? null;
        ...
        /* ⓘ শূন্য মানে বলার কিছু নেই — ব্যাজও নেই। */
