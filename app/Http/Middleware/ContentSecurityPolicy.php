...
class ContentSecurityPolicy
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        /*
         * ⛔ পরিবহনের হেডারগুলো CSP-র সুইচের **উপরে**।
         *
         * ⚠️ ২১ সেপ্টেম্বর ২০২৬: প্রথমে এগুলো নিচে বসানো হয়েছিল, আর
         * তাতে `ABOS_CSP=off` দিলে HSTS ও `nosniff` দুইটাই চুপচাপ চলে
         * যেত। ⓘ সুইচটা বানানো হয়েছিল *"CSP কোনো পাতা ভাঙছে"* অবস্থার
         * জন্য — তখন নিরাপত্তার বাকি দুইটাও সাথে নেভানোর কোনো কারণ নেই।
         *
         * ⭐ পরীক্ষাতেই ধরা পড়ল: পরীক্ষায় CSP বন্ধ থাকে, তাই হেডার
         * দুইটা আসেনি।
         */
    ...
    /**
     * HTTPS-এই থাকতে বলা, আর ধরনটা আন্দাজ করতে মানা।
     *
     * ── ⛔ HSTS কী ঠেকায়, ২১ সেপ্টেম্বর ২০২৬ ───────────────────────────
     * ব্রাউজার প্রথমবার `http://` দিয়ে ঢোকে, সার্ভার `https://`-এ পাঠায়।
     * ⚠️ ঐ প্রথম অনুরোধটা খোলা যায় — কেউ মাঝপথে বসে থাকলে সে ওটাই ধরে,
     * আর সেশন কুকিটা নিয়ে যায়। ⓘ HSTS বলে দেয় *"এই ঠিকানায় আর কখনো
     * http নয়"*, তাই দ্বিতীয়বার থেকে ব্রাউজার নিজেই খোলা পথে যায় না।
     *
     * ── ⚠️ `preload` কেন নেই ────────────────────────────────────────
     * ওটা ব্রাউজারের ভিতরে বেক হওয়া তালিকা, আর **ফেরার পথ নেই** —
     * সাবডোমেইনের কোনোটা কোনোদিন http-এ লাগলে মাসখানেক অচল থাকত।
     * ⓘ `includeSubDomains`ও সেই কারণেই নয়: এখানে একটাই হোস্ট।
     *
     * ── ⛔ কেবল সত্যিকারের HTTPS-এ ─────────────────────────────────────
     * উন্নয়নে `http://localhost`-এ হেডারটা বসালে ব্রাউজার ঐ হোস্টটাকে
     * চিরকালের জন্য https বলে ধরত, আর তখন স্থানীয় কাজ বন্ধ হয়ে যেত —
     * আর সারানোর একমাত্র পথ ব্রাউজারের ভিতরের তালিকা হাতে মোছা।
     */
    private function hardenTransport(Request $request, Response $response): void
    {
        /*
         * ⓘ `nosniff` এখানে, আলাদা মিডলওয়্যারে নয় — দুইটাই "ব্রাউজার
         * যেন নিজে থেকে কিছু ধরে না নেয়" জাতের নিয়ম, আর দুই জায়গায়
         * লিখলে একদিন একটা বসত আর অন্যটা নয়।
         */
