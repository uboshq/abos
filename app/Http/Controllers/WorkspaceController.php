...
class WorkspaceController extends Controller
{
    ...
    public function saveAppearance(Request $request): RedirectResponse
    {
        ...
        /*
         * ⛔ টিকটা কেবল **রূপ বদলালে** খাটে — নাহলে রং বাছাই যায় না।
         *
         * ── কী ঘটছিল, ৫ সেপ্টেম্বর ২০২৬ ─────────────────────────────
         * মালিক: *"রং বাছলে রং পরিবর্তন হয় না।"* ⓘ আর তিনি ঠিক
         * বলেছেন — **প্রতিবার, সবার জন্য**।
         *
         * রঙের রেডিওতে `onchange="this.form.requestSubmit()"` আছে,
         * অর্থাৎ রং বাছলেই **পুরো ফর্মটা** জমা হয় — উপরের
         * `match_accent` টিকসহ। ⚠️ আর ঐ টিকটা ব্লেডে **হার্ডকোড
         * `checked`**, প্রতিবার পাতা খুললেই ফিরে আসে। ⛔ ফলে জমা পড়ত
         * "গোলাপি চাই" + "রূপের রংটাও বসুক", আর দ্বিতীয়টা প্রথমটাকে
         * চাপা দিয়ে navy-র নিজের রং (abos) বসিয়ে দিত।
         *
         * ⓘ কিছুই ভাঙত না, কোনো বার্তা আসত না — সেভ সফল হত, শুধু
         * অন্য মান নিয়ে। **নীরব ভুল**, আর তাই মাসখানেক টিকে ছিল।
         *
         * ── কেন শর্তটা "রূপ বদলেছে" ────────────────────────────────
         * টিকটার মানে ছিল *"আমি যখন একটা রূপ বাছি, তার রংটাও নিও"* —
         * অর্থাৎ ওটা **রূপ বাছার সাথে জড়িত**, রং বাছার সাথে নয়।
         * ⭐ তাই এখন সে কেবল তখনই কথা বলে যখন `ui` সত্যিই বদলাচ্ছে।
         *
         * ⚠️ ফলাফল দুইটাই ঠিক থাকে: Odoo বাছলে অবার্জিন বসে (টিক
         * থাকলে), আর তারপর গোলাপি বাছলে গোলাপিই থাকে — কারণ ঐ জমায়
         * রূপ বদলায়নি।
         */
        $lookChanged = isset($validated['ui'])
            && $validated['ui'] !== $request->user()->ui;

        if ($matchAccent && $lookChanged) {
            /*
