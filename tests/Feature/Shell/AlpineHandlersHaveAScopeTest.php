...
class AlpineHandlersHaveAScopeTest extends TestCase
{
    ...
    public function test_every_ref_a_component_uses_actually_exists(): void
    {
        ...
        foreach ($this->bladeFiles() as $file) {
            ...
            /*
             * ⚠️ মন্তব্য বাদ — নাহলে পাহারাটা **ব্যাখ্যাকে** কোড ভাবে।
             *
             * ── যেভাবে ধরা পড়ল, ৪ সেপ্টেম্বর ২০২৬ ──────────────────
             * সরাসরি বিক্রির পর্দায় দুইটা অভিযোগ আসছিল:
             * `$refs.dueDate` আর `$refs.depositDate`। দুইটাই ছিল JS
             * ব্লক-মন্তব্যের ভিতরে; কোডে ওই নামে কিছু চলেই না।
             *
             * ⛔ আর মন্তব্যটার **বিষয়বস্তুই** ছিল "`$refs` এখানে কাজ
             * করে না, আর কেন" — `x-ref` `x-ui.date` কম্পোনেন্টের নিজের
             * স্কোপে নিবন্ধিত হয়, তাই ফর্মের স্কোপ থেকে সবসময়
             * `undefined`। কোডটা সেজন্যই `Alpine.$data()` ব্যবহার করে।
             * অর্থাৎ পাহারাটা ঠিক সেই লেখাটাকে অপরাধ ধরছিল যেটা
             * বলছে **কেন নিয়মটা ওখানে খাটে না**।
             *
             * ⓘ এই ফাইলটা এই শিক্ষাটা আগেই একবার পেয়েছে —
             * [[hasScope]] ৩০ আগস্ট থেকে ব্লেড-মন্তব্য ছেঁটে নেয়।
             * তখন কেবল `{{-- --}}` ছাঁটা হয়েছিল, কিন্তু Alpine-এর
             * ব্যাখ্যাগুলো থাকে `<script>`-এর ভিতরে, JS-এর ব্লক
             * মন্তব্যে — যেটা তখন ছাঁটা হত না।
             */
            $source = $this->withoutProse($raw);
    ...
    {
        return str_contains($this->withoutProse($source), 'x-data');
    }

    /**
     * কোডটুকু — মানুষের লেখা ব্যাখ্যা বাদ দিয়ে।
     *
     * ── কেন এটা একটাই জায়গায় ───────────────────────────────────────
     * এই ফাইলের দুইটা পাহারাই সোর্সে শব্দ খোঁজে, আর দুইটাই একই ফাঁদে
     * পড়েছে — [[hasScope]] ৩০ আগস্ট ২০২৬-এ, `$refs`-এরটা ৪ সেপ্টেম্বর।
     * আলাদা রাখলে তৃতীয় পাহারাটা আবার একই ভুল করত।
     *
     * ── ⚠️ `//` কেন শুধু পুরো লাইনের ক্ষেত্রে ────────────────────────
     * `//` যেকোনো জায়গা থেকে কাটলে একটা **স্ট্রিংয়ের ভিতরের** `//`-ও
     * ধরা পড়ত — যেমন `'https://…'` — আর তখন ওই লাইনের বাকি আসল
     * কোডটুকু মুছে যেত। ফল হত একটা **ভুয়া সবুজ**: সত্যিকারের
     * `$refs.x` লুকিয়ে যেত, আর পাহারাটা চুপ করে থাকত।
     *
     * ভুয়া লালের চেয়ে ভুয়া সবুজ অনেক খারাপ, তাই এখানে কেবল সেই
     * লাইনগুলো ছাঁটা হয় যেগুলো **পুরোটাই** মন্তব্য — ওখানে লুকানোর
     * মতো কোনো কোড থাকতেই পারে না।
     */
    private function withoutProse(string $source): string
    {
        $prose = [
            '/\{\{--.*?--\}\}/s',   // ব্লেড মন্তব্য
            '/<!--.*?-->/s',        // HTML মন্তব্য
            '/\/\*.*?\*\//s',       // JS ও CSS ব্লক মন্তব্য
            '/^[ \t]*\/\/.*$/m',    // পুরো লাইন জুড়ে লেখা // মন্তব্য
        ];

        foreach ($prose as $pattern) {
