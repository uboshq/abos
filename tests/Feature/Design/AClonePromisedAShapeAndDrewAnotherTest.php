...
class AClonePromisedAShapeAndDrewAnotherTest extends TestCase
{
    ...
    /**
     * বাছাই ক্লিকেই বসে — সংরক্ষণ খুঁজতে হয় না।
     *
     * ── কেন এটা একটা পরীক্ষার যোগ্য দাবি ─────────────────────────────
     * মালিক তিন দিন ধরে বলেছেন "থিম বদলায় না", আর আমি প্রতিবার মেপে
     * দেখিয়েছি সার্ভারে বদলায়। দুইজনেই ঠিক ছিলাম: সার্ভার বদলাত,
     * কিন্তু **বদলাতে দুইটা ধাপ লাগত** — কার্ডে ক্লিক, তারপর নিচে নেমে
     * সংরক্ষণ। কার্ডে ক্লিকের পর কেবল বর্ডারটা বদলাত।
     *
     * ২৯ আগস্ট ২০২৬-এ তিনি Ava দেখিয়ে বললেন ওখানে কত মসৃণভাবে বদলায়।
     * Ava-র থিমের ঘরে `this.form.requestSubmit()` — এক ক্লিক, ব্যস।
     *
     * এখানে দাবিটা তাই আচরণের, রঙের নয়: চারটা বাছাইয়ের প্রতিটাই
     * ক্লিকেই যায়। কেউ একটা `onchange` মুছে দিলে ওই বাছাইটা নীরবে
     * দুই-ধাপে ফিরে যেত, আর অভিযোগটাও ফিরে আসত।
     */
    public function test_every_choice_on_the_appearance_page_applies_on_the_click(): void
    {
        $body = (string) $this->actingAs($this->user)->get(route('appearance'))->getContent();

        $missing = [];

        foreach (['ui', 'accent', 'theme', 'locale'] as $field) {
            /*
             * ওই `name`-এর ইনপুটটা খুঁজে তার ভেতরে `requestSubmit`
             * আছে কি না — গোটা পাতায় শব্দটা আছে কি না দেখলে একটা
             * থাকলেই চারটাই সবুজ হত।
             */
            ...
            if ($found === 0) {
                $missing[] = "{$field} — পাতায় এই বাছাইটাই নেই";
        ...
        $this->assertSame([], $missing, implode("\n", [
            ...
            ...$missing,
            '',
            'ক্লিকের পর পর্দা না বদলালে মানুষ ধরে নেন কিছুই কাজ করেনি —',
            'আর তিন দিন ধরে ঠিক সেটাই বলা হয়েছিল।',
        ]));

        /*
         * বোতামটা তবু থাকে: JavaScript বন্ধ থাকলে ওটাই একমাত্র পথ।
         * ক্লিকে-বসা সুবিধা, একমাত্র উপায় নয়।
         */
        $this->assertStringContainsString('type="submit"', $body,
            'সংরক্ষণ বোতামটা তুলে দেওয়া হয়েছে — JS ছাড়া তখন কিছুই বাছা যাবে না।');
    }
