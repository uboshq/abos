...
class EveryListScreenPaginatesTest extends TestCase
{
    ...
    private const NOT_REALLY_A_LIST = [
        ...
        /* ── সংখ্যার পর্দা: সারি নয়, কার্ড ────────────────────────── */
        'finance.risk' => 'ঝুঁকির বোর্ড — সারির সংখ্যা **গঠনগতভাবে** বাঁধা: সাতটা ঝুঁকি, '
            .'প্রতিটা একটা প্রশ্নের উত্তর (নগদ ফুরাবে কবে, কার বকেয়া বেশি, মজুদ জমে আছে কি না)। '
            .'ব্যবসা যত বড়ই হোক সংখ্যাটা বাড়ে না, কারণ প্রশ্নগুলো কোডে লেখা — '
            .'সারি নয়, উত্তর। ⓘ ভিতরের কোয়েরিগুলো নিজেরাই সীমিত (RiskBoard::risks), '
            .'আর একবারের পূর্বাভাস থেকে তিনটা ঝুঁকি ডেরাইভ হয়',

        'governance.retention.index' => 'নীতির তালিকা — দশটা স্থির সারি, প্রতিটা একটা টেবিলের '
            .'সংরক্ষণ নিয়ম (খতিয়ান, ভাউচার, ব্যাকআপ, রিপোর্ট)। ⓘ সারিগুলো কোডে লেখা '
            .'([[WhatIsKeptHowLong::all]]), ব্যবসার তথ্য থেকে আসে না — তাই ব্যবসা বড় হলে '
            .'সংখ্যাটা এক থাকে, কেবল প্রতিটার পাশের গোনাটা বাড়ে। পাতা ভাগ করলে '
            .'নিরীক্ষকের প্রশ্নের উত্তরটা দুই পাতায় ভাগ হয়ে যেত',

        /*
         * ⭐ রোলের পর্দা আর তালিকা নয় — ২৪ সেপ্টেম্বর ২০২৬।
         *
         * ⓘ মালিকের প্রশ্ন *"ekoi jinis dui porda keno?"*-র পর তালিকা আর
         * সম্পাদনা এক পর্দা হয়েছে ([[RoleController::index()]])। ⚠️ ছাড়টা
         * আগে `STILL_BEING_DONE`-এ ছিল *"abos-5b-র ভাগে"* লেখা — কাজটা
         * হয়ে যাওয়ার পর ঐ কারণটা আর সত্যি নয়, আর **মিথ্যা কারণসহ ছাড়
         * ছাড়হীনতার চেয়ে খারাপ**: সে চিরকাল বসে থাকে।
         */
        'system_admin.role.index' => 'তালিকা নয়, সম্পাদনার পর্দার বাঁ কলাম — রোল বাছার জায়গা, '
            .'দেখার তালিকা নয়। ⓘ সারি বাড়ে প্রতিষ্ঠান কয়টা ভূমিকা **সংজ্ঞায়িত** করেছে তাতে '
            .'(আজ ১৩টা), ব্যবসার আয়তনে নয় — লেনদেন বা মানুষ বাড়লে সংখ্যাটা বাড়ে না। '
            .'⚠️ পাতা ভাগ করলে দ্বিতীয় পাতার রোলগুলো সম্পাদনার সময় বাঁ কলামে থাকত না, '
            .'অর্থাৎ ঐ রোলগুলোয় যাওয়ার দরজাই বন্ধ হত। খোঁজাটা ঐ কলামেই, জীবন্ত',

        'finance.cfo' => 'ড্যাশবোর্ড — দশ-বারোটা কার্ড, প্রতিটা একটা সংখ্যা (নগদ, পাওনা, দেনা, চলতি অনুপাত)। '
            .'কোনো সারির তালিকা নেই, তাই পাতা ভাগের কিছু নেই; সংখ্যাগুলো খতিয়ানে যোগফল, '
            .'আর ব্যবসা বড় হলেও কার্ডের সংখ্যা বাড়ে না',
        ...
        'finance.account_analysis.index' => 'বাঁধা — তিনটা তালিকারই সীমা ডেটার বাইরে: মাসের সারি বাছা সময়ের (বারো-চব্বিশটা), কাগজের ধরন ছকের গোনা কয়টা, আর পক্ষ আগে থেকেই limit(15)। ব্যবসা বাড়লে সারি বাড়ে না, কেবল সংখ্যাগুলো বড় হয়',
    ...
    private const STILL_BEING_DONE = [
        /*
         * ⛔ হাতধারের তালিকা — ২২ সেপ্টেম্বর ২০২৬।
         *
         * ℹ আগে এটা NOT_REALLY_A_LIST-এ ছিল, কারণ হিসেবে লেখা ছিল
         * যে উপরের তিনটা যোগফল এই তালিকা থেকেই গোনা হয়। ⚠️ কিন্তু ওটা
         * **সমাধানযোগ্য**, গঠনগত সীমা নয় — যোগফলটা আলাদা একটা
         * কোয়েরিতে নেওয়া যায়, যেমন ReportEngine নেয়।
         *
         * ⛔ আর সারিগুলো **ব্যবসার সাথে বাড়ে**:
         * `HandLoanAccount::query()->open()->...->get()` — প্রতিটা
         * খোলা হাতধারের একটা করে সারি। ℹ কোডের মন্তব্য বলে
         * সংখ্যাটা দশে গোনা — আর সেটা একটা অনুমান, কোথাও বাঁধা নয়।
         *
         * ⚠️ তাই এটা তথ্য নয়, **ছাড়** — আর ছাড় গোনা হয়
         * ([[EveryExcuseWeGrantedIsCountedTest]])।
         */
        'finance.hand_loan.index' => 'যোগফলটা আলাদা কোয়েরিতে নিলে পাতা ভাগ করা যাবে; সারি বাড়ে খোলা হাতধারের সংখ্যায়',

        /*
         * ⭐ অনুমোদনের দুইটা পর্দা — ২৪ সেপ্টেম্বর ২০২৬।
         *
         * ⓘ দুইটা আলাদা কারণ, আর সেটা মেপে দেখা — একটাই কারণ
         * দুই জায়গায় লিখলে একটা মিথ্যা হত।
         */
        'approval.limit.index' => 'নীতির ছক — সারি বাড়ে প্রতিষ্ঠান কয়টা সীমা **সংজ্ঞায়িত** '
            .'করেছে তাতে (রোল × কাজ × শাখা), লেনদেনে নয় — `system_admin.role.index`-এর হুবহু একই আকার। '
            .'⛔ আর এটা একটা **অগ্রাধিকারের তালিকা**: সবচেয়ে নির্দিষ্ট সারিটা জেতে, '
            .'তাই পাতা ভাগ করলে দ্বিতীয় পাতার একটা সারি প্রথম পাতারটাকে ছাপিয়ে যেত — '
            .'আর পর্দাটা সত্যি বলেও ভুল বোঝাত',

        'approval.exception.index' => 'সীমা — পাতা ভাগ নয়, [[ApprovalExceptions::LOOK_AT]]। '
            .'সারিগুলো ধরন ধরে দলে বাঁধা আর প্রতিটা দলের মাথায় গোনা; পাতা ভাগ করলে '
            .'কাট পড়ত একটা ধরনের মাঝখানে আর মাথার গোনাটা মিথ্যা হয়ে যেত। '
            .'ⓘ `approval.inbox.index`-এর ছাঁচ: সীমা + পর্দায় লেখা থাকে কতটা দেখা হয়েছে',
