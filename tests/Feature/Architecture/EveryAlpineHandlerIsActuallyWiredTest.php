...
final class EveryAlpineHandlerIsActuallyWiredTest extends TestCase
{
    ...
    private function handlersIn(string $dir): array
    {
        ...
        foreach (File::files($dir) as $file) {
             ...
             * জিনিস, ডাকার নয়।
             *
             * ── ⛔ আর ভাষার নিজের শব্দগুলোও বাদ — ২২ সেপ্টেম্বর ২০২৬ ──
             * ⚠️ ছাঁচটা দেখে **`if (! form || form.tagName !== 'FORM') {`**
             * আর ভাবত হ্যান্ডলারের নাম `if`। ⓘ আট ফাঁকা দিয়ে শুরু, বন্ধনীর
             * ভিতরে কোনো `)` নেই — সব শর্ত মিলে যায়।
             *
             * ⛔ ফলে পাহারাটা **মিথ্যা লাল** দিত: *"`if()` কোথাও ব্যবহার
             * হয় না"*। ⚠️ আর সেটা পড়ে কেউ `actions.js` থেকে "মৃত কোড"
             * মুছতে যেতেন — অথচ মোছার মতো কিছুই নেই, ওটা একটা শর্ত।
             *
             * ⓘ মিথ্যা লাল মিথ্যা সবুজের চেয়ে কম বিপজ্জনক নয়: কিছুদিন
             * পরে কেউ আর পাহারাটা পড়ে না, আর তখন আসল লালটাও চাপা পড়ে।
             */
            preg_match_all(
                '/^\s{8}(?!(?:get|if|for|while|switch|catch|return|function)\b)'
                .'([a-z][A-Za-z0-9]*)\s*\([^)]*\)\s*\{/m',
                $src,
                $m,
            );
