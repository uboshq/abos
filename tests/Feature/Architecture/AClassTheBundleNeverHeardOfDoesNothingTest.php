...
class AClassTheBundleNeverHeardOfDoesNothingTest extends TestCase
{
    ...
    public function test_every_class_a_blade_writes_exists_in_the_bundle(): void
    {
        ...
        /*
         * ⛔ মেঝেটা ৫০০ ছিল, আর সেটা ঢিলা ছিল — ২১ সেপ্টেম্বর ২০২৬।
         *
         * ⚠️ সেদিন হাতে লেখা একটা স্ক্রিপ্ট শেল-কোটিংয়ে ব্যাকস্ল্যাশ
         * খেয়ে ফেলে ১০৯৬-এর মধ্যে মাত্র ৬৪২টা শ্রেণি গুনেছিল — আর
         * ৬৪২ এই মেঝেটা দিব্যি পার হয়ে যেত।
         *
         * ⓘ ফল হত উল্টো বিপদ: পাহারাটা ৪০% শ্রেণিকে "বান্ডিলে নেই"
         * বলত, আর কেউ সেই তালিকা ধরে **কাজ করা** শ্রেণি মুছে ফেলত।
         *
         * ⭐ তাই মেঝেটা এখন আসল সংখ্যার কাছাকাছি। বান্ডিল ছোট হলে
         * এটা লাল হবে — আর সেটাই চাই: ছোট হওয়ার কারণটা আগে দেখা হোক।
         */
    ...
    private function bundleClasses(): array
    {
        ...
        foreach ($sheets as $file) {
            /*
             * ⛔ অঙ্ক দিয়ে শুরু হওয়া নাম — ২৫ সেপ্টেম্বর ২০২৬।
             *
             * ── ⚠️ এই পাহারাটা নিজেই অন্ধ ছিল ────────────────────────
             * CSS-এ শ্রেণির নাম অঙ্ক দিয়ে শুরু হতে পারে না, তাই Tailwind
             * `2xl:grid-cols-4` লেখে `.\32 xl\:grid-cols-4` রূপে —
             * ব্যাকস্ল্যাশ, হেক্স কোড-পয়েন্ট, **আর escape শেষ করার একটা
             * স্পেস**।
             *
             * ⛔ পুরনো নিয়মে ঐ স্পেসটা কোনো শাখাতেই ছিল না, তাই মিলটা
             * `\32`-এর পরেই থেমে যেত আর বাকি `xl:grid-cols-4` হারাত।
             * ⚠️ ফল: **অঙ্ক দিয়ে শুরু হওয়া প্রতিটা শ্রেণি বান্ডিলে থেকেও
             * "নেই" বলে অভিযোগ পেত**।
             *
             * ⓘ আর ক্ষতিটা কাল্পনিক নয়: ২২ সেপ্টেম্বর একই ভাঙা মাপ থেকে
             * সিদ্ধান্ত হয়েছিল *"গোটা `2xl:` ভ্যারিয়েন্টটাই তৈরি হয় না"*,
             * আর সেই ভিত্তিতে একটা পর্দার শ্রেণি বদলানো হয়েছিল —
             * [[role/form.blade.php]]-এর মন্তব্যে "দুইবার মেপে দেখা"
             * সিলমোহর সহ। ⚠️ দুইবারই একই ভাঙা যন্ত্রে।
             *
             * ⓘ সূত্রটা ছিল অসামঞ্জস্যে: একই লাইনের `md:` আর `xl:`
             * অভিযোগে আসত না, কেবল `2xl:` আসত।
             */
            preg_match_all('/\.((?:[a-zA-Z0-9_-]|\\\\[0-9a-fA-F]{1,6} ?|\\\\.)+)/', File::get($file), $found);

            foreach ($found[1] as $name) {
                /*
                 * ⓘ হেক্স escape-টা আগে আসল অক্ষরে ফেরানো হয়, তারপর
                 * বাকি ব্যাকস্ল্যাশগুলো ছাঁটা — ⚠️ উল্টো ক্রমে করলে
                 * `\32 ` থেকে `32 ` থেকে যেত, আর নামটা তবু মিলত না।
                 */
                $name = preg_replace_callback(
                    '/\\\\([0-9a-fA-F]{1,6}) ?/',
                    fn (array $m) => chr(hexdec($m[1])),
                    $name,
                );
