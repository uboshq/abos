...
class PublicIdTest extends TestCase
{
    ...
    private const FRAMEWORK = [
        ...
        /*
         * দ্বৈত-জমার টোকেন ([[FormIsNotSubmittedTwice]], ১৩ সেপ্টেম্বর ২০২৬)।
         *
         * সারিটা একটা **ফর্মের একটা রেন্ডার** — ব্যবসার কোনো নথি নয়।
         * বাইরের কী লাগে সেই সারিগুলোর যেগুলো API বা ঠিকানায় নাম ধরে
         * চাওয়া হয়; এটা কেউ কখনো চাইবে না। ⓘ আর এর নিজের `token`
         * কলামটাই ইতিমধ্যে একটা uuid7 আর unique — দ্বিতীয় একটা বাইরের
         * কী মানে একই কাজের দুইটা চাবি।
         *
         * ⚠️ সারিগুলো রোজ ছাঁটা হয়, তাই এদের কোনো দীর্ঘ জীবনও নেই।
         */
        'submitted_forms',

        /*
         * খতিয়ানের চেইনের মাথা ([[LedgerChain]], ২ সেপ্টেম্বর ২০২৬)।
         *
         * ── কেন এখানে, আর কেন `publicId()` যোগ করা ভুল হত ────────────
         * এটা ব্যবসায়িক সারি নয়, **যন্ত্রের নিজের হিসাবের কাগজ**:
         * কোম্পানি প্রতি ঠিক একটা সারি, যাতে শেষ ছাপ ও গোনা সংখ্যা
         * থাকে। কেউ কোনোদিন একটা "চেইনের মাথা" নাম ধরে চাইবে না —
         * কোম্পানিটা চাইবে, আর কোম্পানির নিজের কী আছে।
         *
         * টেবিলটার প্রাথমিক চাবি `company_id`, আলাদা কোনো `id` নেই।
         * `public_id` বসালে সেটা এমন একটা সারির বাইরের নাম হত যেটা
         * বাইরে থেকে চাওয়ার কোনো পথই নেই — একটা কলাম যা কোনোদিন
         * পড়া হয় না, আর পরেরজন ভাবতেন কোথাও একটা API আছে।
         */
        'ledger_chain_heads',
    ];
