...
class EveryRawQueryNamesItsCompanyTest extends TestCase
{
    ...
    private const DECLARED = [
        ...
        /*
         * ⭐ ভাই-কোম্পানির লেনদেন — ২৫ সেপ্টেম্বর ২০২৬।
         *
         * ── ⛔ এখানে ছাঁকনি বসানোই ভুল হত ────────────────────────────
         * ⓘ [[InterCompanyService::kindsOf()]] পড়ে `in_voucher_id`-র
         * লাইনগুলো, আর ঐ ভাউচারটা **ইচ্ছাকৃতভাবে অন্য কোম্পানির** —
         * সেটাই তো আন্তঃকোম্পানি লেনদেনের মানে।
         *
         * ⚠️ `->where('company_id', CompanyContext::id())` বসালে
         * ফাংশনটা **সবসময় খালি** ফিরত, আর পর্দা নীরবে প্রতিটা সারিতে
         * "এখনো বসেনি" দেখাত — কোনো ত্রুটি ছাড়াই।
         *
         * ── ⭐ তাহলে দেয়ালটা কোথায় ─────────────────────────────────
         * ⓘ আইডিগুলো আসে [[InterCompanyTransfer]] সারি থেকে, আর সে
         * [[BelongsToCompany]]-র স্কোপে বাছা — অর্থাৎ ছাঁকনিটা **উপরের
         * ধাপে** বসেই গেছে। ⚠️ এখানে দ্বিতীয়বার বসালে সেটা দেয়াল নয়,
         * ফিচারটার মৃত্যু।
         *
         * ⓘ abos-3c ও abos-13 দুইজনেই স্বাধীনভাবে একই পড়া দিয়েছেন।
         */
        'app/Modules/Accounts/Services/InterCompanyService.php' => 'ভাই-কোম্পানির লেনদেনের ধরন গোনা — `in_voucher_id` ইচ্ছাকৃতভাবে '
            .'**অন্য কোম্পানির** ভাউচার, আর আইডিগুলো কোম্পানি-স্কোপে বাছা '
            .'`acc_inter_company` সারি থেকেই আসে; এখানে ছাঁকনি বসালে উত্তর সবসময় খালি হত',

        /*
         * ⭐ কাগজ সংরক্ষণের পর্দা — "এই টেবিলে কত সারি" গুনতে টেবিলের নামটা
         * **চলক** হিসেবে আসে, কারণ একই কোড নয়টা আলাদা টেবিল গোনে।
         *
         * ⓘ ছাঁকনিটা বসানো আছে ([[WhatIsKeptHowLong::rowsOf]]) — শর্তসাপেক্ষে,
         * কারণ `submitted_forms`-এ `company_id` কলামই নেই (ওটা ফর্ম দুইবার
         * জমা ঠেকানোর চিহ্ন, ব্যবসার তথ্য নয়)। ⚠️ গার্ডটা চলক-নাম ধরে
         * টেবিলটা চিনতে পারে না, তাই কলাম আছে কি না সেও দেখতে পারে না।
         *
         * ⛔ ২০ সেপ্টেম্বর ২০২৬ পর্যন্ত এখানে কোনো ছাঁকনিই ছিল না, আর তাতে
         * এক কোম্পানির লোক অন্য কোম্পানির ব্যবসার **আকার** পড়ে ফেলতেন —
         * কত ভাউচার, কত খতিয়ান সারি।
         */
        'app/Modules/Governance/Services/WhatIsKeptHowLong.php' => 'টেবিলের নাম চলক (নয়টা টেবিল এক কোডে); ছাঁকনি '
            .'`rowsOf()`-এ শর্তসাপেক্ষে বসানো, কারণ একটা টেবিলে `company_id` কলামই নেই',
