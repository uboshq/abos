...
/**
 * গ্রাহকের পর্দা নিজে কোয়েরি লেখে না — সরু পথে জিজ্ঞেস করে।
 *
 ...
 *
 * ── কী বসানো হলো ─────────────────────────────────────────────────────
 * সব কোয়েরি এখন [[App\Modules\Sales\Services\CustomerPapers]]-এ, আর তার
 ...
 * গ্রাহকের নয়)। আওতাটা আসে **দরজা ধরে**: যে রুটগুলোয় `auth:portal`
 * বসানো, তাদের কন্ট্রোলার।
 */
class EveryPortalScreenAsksTheNarrowPathTest extends TestCase
{
    /**
     * গ্রাহকের দরজার পিছনে যে কয়টা সরাসরি মডেল-ডাক টিকে আছে — আর কেন।
     *
    ...
    private const HANDLED = [
         ...
         *
         * [[App\Modules\Sales\Services\CustomerPapers]]-এর ভিত্তি-নিয়ম:
        ...
        'PortalController.php → withoutGlobalScopes()' => [1, 'ওই একই লগইনের খোঁজ — কোন কোম্পানির গ্রাহক, সেটাই তো এখনো অজানা'],

        /*
         * ব্যাংকের তালিকা — এটা কারো কাগজ নয়।
         *
         * জমার দাবি তুলতে গ্রাহককে বলতে হয় "কোন ব্যাংকে দিয়েছি", আর
         * তালিকাটা **আমাদের নিজের** হিসাব — প্রতিটা গ্রাহকের জন্য এক।
    ...
    public function test_a_portal_screen_does_not_write_its_own_query(): void
    {
        ...
        foreach ($found as $key => $times) {
            $this->assertArrayHasKey($key, self::HANDLED,
                "\n\nগ্রাহকের পর্দা নিজে কোয়েরি লিখছে: {$key}\n\n"
                ."কোয়েরিটা CustomerPapers-এ নিন। ওখানে কোনো পদ্ধতি \"কার\" জিজ্ঞেস করে না —\n"
                ."গ্রাহক আসে গার্ড থেকে, তাই ভুল আইডি পাঠানোর উপায় থাকে না।\n\n"
        ...
        /*
         * ⛔ গোয়েন্দাটা সত্যিই তাকিয়েছে কি না — ২২ সেপ্টেম্বর ২০২৬।
         *
         * ── ⚠️ উপরের সবকিছু ছিল, তবু পাহারাটা অন্ধ হতে পারত ─────────
         * `directCallsIn()`-কে খালি ফিরিয়ে দিলে `$found` খালি হয়, উপরের
         * দুইটা লুপ **শূন্যবার** চলে, আর টিকে থাকে কেবল বাজেটের দাবিটা —
         * যেটা দুইটা ধ্রুবকের তুলনা, কোডের সাথে যার কোনো সম্পর্ক নেই।
         *
         * ⛔ মেপে দেখা হয়েছে: গোয়েন্দা পুরো অন্ধ করে দিলেও এই ফাইল
         * **সবুজ** থাকত। একমাত্র চিহ্ন ছিল assertion-এর সংখ্যা ১২ থেকে
         * ৬-এ নামা, আর সেটা কেউ পড়ে না।
         *
         * ⚠️ ফলটা কেবল একটা নীরব পরীক্ষা নয়: এই সুইপটা **গ্রাহক-পোর্টালের
         * ফাঁস ঠেকায়**, আর সে "সব পরিষ্কার" বলত কিছু না দেখেই।
         *
         * ── ⭐ ঘোষিত ছাড়গুলোই নমুনা ────────────────────────────────
         * ⓘ ওগুলো **আজ কোডে সত্যিই আছে** বলেই ঘোষিত। তাই একটাও খুঁজে
         * না পাওয়া মানে দুইটার একটা: গোয়েন্দা অন্ধ, নয়তো ছাড়টা বাসি।
         * ⭐ দুইটাই জানা দরকার, আর দুইটাই এই এক দাবিতে ধরা পড়ে।
         */
        foreach (array_keys(self::HANDLED) as $declared) {
            $this->assertArrayHasKey($declared, $found, implode("\n", [
                "\n⛔ ঘোষিত ছাড়টা কোডে আর খুঁজে পাওয়া যাচ্ছে না: {$declared}",
                '',
                'দুইটার একটা ঘটেছে —',
                '',
                'ⓘ ডাকটা সত্যিই সরানো হয়েছে (সেবায় নেওয়া হয়েছে): তাহলে',
                '   HANDLED থেকে সারিটা মুছুন, আর বাজেটের সংখ্যাটাও কমান।',
                '   ছাড়ের তালিকা কেবল কমে।',
                '',
                '⛔ নয়তো গোয়েন্দাটা ভেঙে গেছে — `directCallsIn()` আর কিছুই',
                '   দেখছে না। তখন এই সুইপ "সব পরিষ্কার" বলবে অথচ পোর্টালের',
                '   প্রতিটা নতুন সরাসরি কোয়েরি নীরবে পার পেয়ে যাবে।',
            ]));
        }
    }
     ...
     * গ্রাহক অন্যের দাবিটা পড়তে পারতেন।
     */
    public function test_a_model_that_came_from_the_url_is_checked_against_its_owner(): void
    {
        ...
        foreach ($this->portalActions() as [$class, $method]) {
            ...
            foreach ($reflection->getParameters() as $parameter) {
                ...
                $this->assertMatchesRegularExpression(
                    ...
                    ."কোথাও দেখা হচ্ছে না ওটা এই গ্রাহকেরই কি না।\n\n"
        ...
        $this->assertGreaterThan(0, $bound,
            'গ্রাহকের কোনো পর্দাই ঠিকানা থেকে মডেল নিচ্ছে না — তাহলে এই পাহারাটা কিছুই দেখছে না।');
    }

    // ── খুঁটিনাটি ────────────────────────────────────────────────────

    /**
     * গ্রাহকের দরজার পিছনের প্রতিটা কাজ।
     *
